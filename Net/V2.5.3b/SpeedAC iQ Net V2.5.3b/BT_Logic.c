...
for(i=37;i<=MaxArrayIODbase;i++) 
  {
    ...
    if(i == O_Fault || i == O_LowLevel || (i == O_BagHolder && WP.WeighmentsPerUnit > 1) )  //SADM2 DECEMBER 2020 
     DoNoReset = ON;
    else
     Output_OFF(i);
 ...
 if(T_Dn(T_FirstScan))
  {
   ...
   if (IO_Enable[I_PressureSW] && !Input_State(I_PressureSW) && !FaultState(FltG_LowAirPressure)) 
    ...
    {
      if ((NetWeight[Scale1] > MaxWeight[Scale1]) && !FaultState(FltG_ExceededCapacity)&& (SeqMaster != stp_FEED || (NetWeight[Scale1] > TargetWeight[Scale1]*OverloadFactor)))  //SADM2 2020)
        {
...
if (ChangeWPInStopMode) //SADM2
 ...
 if(SeqManDischarge == stp_INIT && T_Dn(T_BagRelease) && SeqSpout != stp_DONE)
 {
  ...
  SampledInPauseMem = SampledInPause;
    ...
    switch (SeqModeManager)
    {
      ...
      case stp_STOPPING_MODE:
            ...
            if(SeqMaster==stp_INIT && (WeighPerUnitAcc == 0 || CleanoutWeighment) && SeqKicker==stp_INIT)
              {
...
switch (SeqSpout)
       ...
       case stp_DONE:
            ...
            { 
             if(IO_State[O_BagRelease] && !Input_State(I_DischReq)) 
              {
               Output_OFF(O_BagRelease);
              }
