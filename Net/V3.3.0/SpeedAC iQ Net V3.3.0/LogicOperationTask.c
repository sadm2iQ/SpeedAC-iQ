...
void Status_Refresh(void)
{
 ...
 //Re-initialize iQFeed Learning
  ...
  if(WP.FeedAlgoType == FeediQSmart)LearniQSmartParam=ON;
  if(WP.FeedAlgoType == FeediQFlash)LearniQFlashParam=ON;
  if(WP.FeedAlgoType == FeedGHSSmartPlus)LearnSmartPlusParam=ON;
  }
  ...
  req_slow_fill_time = ((MP.SmartStabWindow - 0.05) * step_slow_fill_percentage * WP.SlowFillFactor) + 0.05;
  ...
  if(ActuatorPosMem  != WP.ActuatorPos) 
  {
   ActuatorPosMem = WP.ActuatorPos;
   CopyLearnToNovFlag = Execute;
   if(WP.FeedAlgoType == FeediQSmart)LearniQSmartParam=ON;
  } 
  ...
  if(WP.FeedAlgoType == FeedGHSSmartPlus)LearnSmartPlusParam = CheckiQFeedLearn();
  ...
  if((WP.BulkMode == ByTime && WP.FeedAlgoType == FeedStd) || WP.FeedAlgoType == FeedGHSSmartPlus) BulkOnTime[Scale1] = ON;
...
string TenthOfSecString[2],HeartBeatTime[10][10];
   ...
   if (ProdDataReady)
     {
      PrintUnitRecord();
      ProdDataReady = OFF;
      T_EnOff(T_DelayEthernetSend); //SADM2
     }
     else //SADM2 
     {
      if(!T_EN[T_DelayEthernetSend])
       T_EnOn(T_DelayEthernetSend);
      
      if(T_Dn(T_DelayEthernetSend))
      {
         T_EnOff(T_DelayEthernetSend);
         
         TenthOfSecString = concat(".",trim(string(TenthOfSec)));
         //Print String on Ethernet - channel 2500 always
         PrtPort = ETHERNET_UNITRECORD_CHANNEL; PrtPortPara = concat ("",":2500");
         SendUnitRecToPort(PrtPort,PrtPortPara,concat("HEARTBEAT iQ TIME-",TimeString,TenthOfSecString));
         
         for(i=9;i>=1;i--)
         {
          HeartBeatTime[i] = HeartBeatTime[i-1];
         }
         HeartBeatTime[0] = concat(TimeString,TenthOfSecString);
      }   
     }
