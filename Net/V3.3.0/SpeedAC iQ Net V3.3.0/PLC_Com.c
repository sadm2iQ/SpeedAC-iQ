...
ChangeWP         = 32,
SetOutputs       = 33,
ChangeOverfill   = 34,  //SADM2 2020 NOVEMBER 
}
...
/*------------------------------------------------------------------------- 
--  Procedure Name    : Set_Outputs
--  Created by        : SADM2
--  Last modified on  : 2020/10
--  
--  Purpose           : Function used to set an output 
--                    : 
--  Value Parameters  :
--  Variable Params   : Code 100 = invalid instructions
                      : Code 101 and more instruction not possible 
...
int Tempo, err;
  ...
  switch (IN_Value)
  {
    ...
    case CatchGate: //Toggle_CatchGate     
                                      
                              if (mode_step == stp_OFF_MODE) //|| (Current_Screen = SCREEN_IOTEST_IN1 || Current_Screen = SCREEN_IOTEST_IN2 || Current_Screen = SCREEN_IOTEST_IN3))
                               {
                               ...
                               else if (IO_State[O_CatchGate]) 
                                    {
                                      Output_OFF(O_CatchGate);
                                      Tempo = CatchGate;
                                    }
                                    else Tempo = CatchGate+100;
                               }
       break;
       
    case BulkFeed: //Toggle_BulkFeed     
                                      
                              if (mode_step == stp_OFF_MODE )
                               {
                               if(!IO_State[O_BulkFeed]) 
                               {
                                 Output_ON(O_BulkFeed);
                                 Tempo = BulkFeed;
                               }
                               else if (IO_State[O_BulkFeed]) 
                                    {
                                      Output_OFF(O_BulkFeed);
                                      Tempo = BulkFeed;
                                    }
                                    else Tempo = BulkFeed+100;
                               }
       break;
       
    case DribbleFeed: //Toggle_DribbleFeed     
                                      
                              if (mode_step == stp_OFF_MODE )
                               {
                               if(!IO_State[O_DribbleFeed]) 
                               {
                                 Output_ON(O_DribbleFeed);
                                 Tempo = DribbleFeed;
                               }
                               else if (IO_State[O_DribbleFeed]) 
                                    {
                                      Output_OFF(O_DribbleFeed);
                                      Tempo = DribbleFeed;
                                    }
                                    else Tempo = DribbleFeed+100;
                               }
       break;
       
    case DischDoor: //Toggle_DischDoor     
                              ...
                              if (mode_step == stp_OFF_MODE )
                               ...
                               else if (IO_State[O_DischDoor]) 
                                    {
                                      Output_OFF(O_DischDoor);
                                      Tempo = DischDoor;
                                    }
                                    else Tempo = DischDoor+100;
                               }
       break;
       
    case StreamRegulator: //Toggle_StreamRegulator     
                              ...
                              if (mode_step == stp_OFF_MODE )
                               ...
                               if(!IO_State[O_StreamRegulator]) 
                               {
                                 ...
                                 Tempo = StreamRegulator;
                               }
                               else if (IO_State[O_StreamRegulator]) 
                                    {
                                      Output_OFF(O_StreamRegulator);
                                      Tempo = StreamRegulator;
                                    }
                                    else Tempo = StreamRegulator+100;
                               }
       break;
...
void Display_AdditionalLiveInfo(void)
{
  int DW_Pointer; 
     ...
     DW_Pointer++;
     return_value[DW_Pointer] = WP.UnderWeight;                //50
     DW_Pointer++;
     return_value[DW_Pointer] = WP.OverWeight;                 //52
...
int DW_Pointer; 
     ...
     DW_Pointer++;
     return_value[DW_Pointer] = ProdStdDev[Scale1];
     DW_Pointer++;
     return_value[DW_Pointer] = ProdLastChecked[Scale1]; 
     DW_Pointer++; 
     ...
     DW_Pointer++;                       
     return_value[DW_Pointer] = ProdCycleUnitsPerMin[Scale1];
     DW_Pointer++; 
...
int DW_Pointer; 
     ...
     DW_Pointer++;
     return_value[DW_Pointer] = adj((WP.Cutoff-CutoffCorrOffset[Scale1]),7,3);
     DW_Pointer++;
     ...
     DW_Pointer++;
     return_value[DW_Pointer] = WP.IntermCutoff;
     DW_Pointer++;
...
int DW_Pointer; 
     ...
     DW_Pointer++; 
     return_value[DW_Pointer] = WP.MinFeeding;
     DW_Pointer++;                                     
     return_value[DW_Pointer] = WP.MaxFeeding;
     DW_Pointer++;
     return_value[DW_Pointer] = WP.RegMinFeeding;
     DW_Pointer++;
     return_value[DW_Pointer] = WP.RegMaxFeeding;
     DW_Pointer++;
     return_value[DW_Pointer] = WP.StreamRegConfig;
     DW_Pointer++; 
     return_value[DW_Pointer] = WP.ActuatorPos;
     DW_Pointer++; 
     return_value[DW_Pointer] = WP.WeighmentsPerUnit;
     DW_Pointer++;
               ...
               if(IN_ValueMem == IN_Value && IN_CommandMem == IN_Command && IN_ParameterMem == IN_Parameter)
                ...
                {             
                         switch (IN_Parameter)
                         {
                          ...
                          case SetOutputs:
                            if (IN_Value > 0 && IN_Value < ParameterMax)
                            return_value[0] = Set_Outputs (IN_Value);
                            else return_value[0] = -1;
                            Clear_DataWords ();
                            break;
                            
                          case ChangeOverfill: //ChangeOverfill
                           if(SeqModeManager == stp_OFF_MODE  || SeqModeManager == stp_STOP_MODE || SeqModeManager == stp_STOPPING_MODE || SeqModeManager == stp_MANUAL_MODE || SeqModeManager == stp_PREP_MODE || SeqModeManager == stp_AUTO_MODE)
                            {
                             if (IN_Value != MemOverfill)
                              {
                                MemOverfill = IN_Value;
                                WP.Overfill = IN_Value;
                                ChangeOverfillinDB(MemOverfill,IN_Value);
                                return_value[0] = ChangeOverfill;
                              }
                              else return_value[0] = ChangeOverfill + InvalidNbr;
                            }
                            else return_value[0] = ChangeOverfill + InvalidNbr;
                            break;
