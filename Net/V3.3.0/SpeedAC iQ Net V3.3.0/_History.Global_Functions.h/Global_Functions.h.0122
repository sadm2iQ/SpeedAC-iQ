...
// Support Email
string CheckEmailSupport(string Email)
{
string EmailField[2][48],EmailTempo[48];
int i,EmailFieldInt,EmailFieldLen;

EmailTempo = "";
split(EmailField,Email,'@');

for(i=0;i<=47;i++)
 {
  EmailFieldInt = int(EmailField[i]);
  
  EmailFieldTest[0] = EmailField[0];
  EmailFieldTest[1] = "@" + EmailField[1];

  if(EmailFieldInt>='@')
   {
    EmailFieldLen = length(EmailField[i]);
    
    if(EmailFieldLen>=1 && EmailFieldLen<=21)
     {
      EmailTempo = concat(EmailTempo,EmailField[i]);
      if(i<21)  EmailTempo = concat (EmailTempo,"@");
     }
     else 
     { 
      EmailTempo = "";
...
//SADM2
// Parts Email
string CheckEmailParts(string PartsEmail)
{
string PartsEmailField[2][48],PartsEmailTempo[48];
int i,PartsEmailFieldInt,PartsEmailFieldLen;

PartsEmailTempo = "";
split(PartsEmailField,PartsEmail,'@');

for(i=0;i<=47;i++)
 {
  PartsEmailFieldInt = int(PartsEmailField[i]);
  
  PartsEmailFieldTest[0] = PartsEmailField[0];
  PartsEmailFieldTest[1] = "@" + PartsEmailField[1];

  if(PartsEmailFieldInt>='@')
   {
    PartsEmailFieldLen = length(PartsEmailField[i]);
    
    if(PartsEmailFieldLen>=1 && PartsEmailFieldLen<=3)
     {
      PartsEmailTempo = concat(PartsEmailTempo,PartsEmailField[i]);
      if(i<3)  PartsEmailTempo = concat (PartsEmailTempo,"@");
     }
     else 
     { 
      PartsEmailTempo = "";
...
T_PRE[T_DelayEthernetSend] = 0.5; //SADM2
T_PRE[T_1Sec]              = 1.0;
...
Recipe_Grayout[33]= !(WP.FeedAlgoType == FeediQSmart || WP.FeedAlgoType == FeedGHSSmartPlus);
Recipe_Grayout[34]= !(WP.FeedAlgoType == FeediQSmart || WP.FeedAlgoType == FeediQFlash || WP.FeedAlgoType == FeedGHSSmartPlus) ;
...
int err;
   ...
   if(WPdb.NominalWeight != WP.NominalWeight || WPdb.Unit != WP.Unit) 
     ...
     if(WPdb.Overfill != WP.Overfill)  //SADM2 JAN 2021
     {
     LoadOverfillFromInputsFlag = Execute;
     }
    ...
    case SCREEN_RECIPE3:  
      ...
      //  Header
        ...
        // Second Column                               
              ...
              widget_text       (RECIPE_ROW32_SELECT, lsub(TX_WPALGOGHSSMARTPLUS), 5);                                              
              // 3rd Row Info 
    ...
    case SCREEN_INFO:
      ...
      // First Column
       ...
       // Second Column     
           ...
           // E-mail
           widget_text (INFO_ROW11_ID, lsub(TX_INFO_EMAIL));
           
           // E-mail
           widget_text (INFO_ROW12_ID, lsub(""));
           ...
           // E-mail
           widget_text (INFO_ROW15_ID, lsub(TX_INFO_EMAIL));
           
           // E-mail
           widget_text (INFO_ROW16_ID, lsub(""));
...
/*------------------------------------------------------------------------------- 
--  Procedure Name    : ChangeOverfillinDB
--  Created by        : SADM2
--  Last modified on  : 2020/11
--  
--  Purpose           : Load new Overfill value while presaving and post-saving infos
--                      
--                                 
--  Value Parameters  :
--  Variable Params   : None
...
void ChangeOverfillinDB(int Old,int New)  //SADM2 JAN 2021
 {
    PreSave_Data(Old,ON);
    WPdb.Overfill = Old;
    find_db (WP_Db); //command to load the active # desired from the recipe dbase                              
    LoadDb2WP();
    MemOverfill = WPdb.Overfill;
    PostLoad_Data();
    //LoadBatch(MemRecipe);
    Batch.Count = 0;
    WP.FeedAlgoType = ValidateAlgoType(MC.FeederType,WP.FeedAlgoType,WP.FeedAlgoType);
...
void InitDBs(void)
{
  ...
  WPdb.No = MemRecipe;
  MemOverfill = WP.Overfill;  //SADM2 JAN 2021
  LoadDb2WP();
  PostLoad_Data();
  LoadBatch(RecipeMem);
  MoveFlexIoDbs2Vars();
