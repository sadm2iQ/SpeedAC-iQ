...
void DisplayHandlingLogic(void)
{
 ...
 int i;
    ...
    if (SystemCleanMem != SystemClean)
    if (Current_Screen == SCREEN_GOTO)
    {
      switch (SystemClean)
      {
        case OFF:   widget_image (GOTO_FCT_BUTTON_3, BTN_SYSTEM_CLEAN);          
                           break;                   
        case ON:    widget_image (GOTO_FCT_BUTTON_3, BTN_SYSTEM_CLEAN_ON);         
                           break;
        default :          widget_image (GOTO_FCT_BUTTON_3, BTN_SYSTEM_CLEAN);
                           break;                    
      }
     SystemCleanMem = SystemClean;
     SystemCleanFct(SystemClean);
    }
...
//Refresh Fault Screen 
    ...
    if(LoadOverfillFromInputsFlag != Done)  //SADM2 JAN 2021
     {
      ChangeOverfillinDB(MemOverfill,IN_Value);
      LoadOverfillFromInputsFlag = Done;
     }
