...
result1 = iQSetEnableSignalOutput (scale_channel, scale_number, -1);
...
bool noerr1,noerr2,EnableBulk,EnableDribble,EnableGate;
int scale_channel,scale_number;
...
noerr2 = ON;
   ...
   if (EnableGate)
    {
    noerr1 = iQSetEnableSignalOutput (scale_channel, scale_number, AddressGate-PIMoffset);
...
// Initialize filling parameters
   ...
   { 
    if(iQClearControlParameters (scale_channel, scale_number))
     ...
     noerror2 = iQSmSetControlParameters(scale_channel,scale_number,CPSmFastAvgPointsTemp,CPSmSlowAvgPointsTemp,CPSmSlowItpPointsTemp,CPSmSwitchTimeTemp);
     scanmem[6] = systemtimer_ms();
     }
   ...
   if(!noerror || !noerror2 || !noerror3) 
    {
     SystemErrorCode = concat("#",trim(string(scale_number)),"-","130 iQ Smart variables not initialized");
    }
...
//Start iQ Feeding
noerror = iQFeedingStart (scale_channel, scale_number, iQFeedWaitSettled[ScaleNo-1]);   // true => activate parameter waiting for settled
return(noerror);
}
...
//Read Data from iQ Feed after feeding completed
 ...
 if (msg_no >= 0)
 {
    iQReadEventStatus = iQReadEvent (ScaleNo,weight,status,fastCO,slowCO,slow_fill_duration,
  ...
  if(!emptybuffer)
   {  
    ...
    { 
     ...
     iQReadEventStats (ScaleNo,stat_controlled_fast_time[ScaleNo-1], stat_controlled_slow_time[ScaleNo-1], stat_stable_fast_flow[ScaleNo-1],
                           stat_stable_slow_flow[ScaleNo-1], stat_fast_flow_fluctuation[ScaleNo-1], stat_slow_flow_fluctuation[ScaleNo-1]);
