...
#include "SpeedACiQ_Duplex_V3.3.0.h"
...
#define BULK1ADDRESS 6
#define BULK2ADDRESS 7
#define PIM3         2
#define PIM4         3
...
#define IO_OUT_MAX     84
#define NUMBEROFINPUTS  36
#define NUMBEROFOUTPUTS 48
#define MAX_FLEXIO      3
#define MAX_DB          5
#define T_ARRAY        90
#define MAXUSERS        3
...
#define InvalidNbr     100
...
#ifndef COM3_CONTOUT_CHANNEL
  #define COM3_CONTOUT_CHANNEL     3
#endif 

#ifndef COM4_CONTOUT_CHANNEL
  #define COM4_CONTOUT_CHANNEL     4
#endif 
  ...
  #define ETHERNET_UNITRECORD_CHANNEL 14
#endif

#ifndef ETHERNET_PRINTREPORT_CHANNEL
  #define ETHERNET_PRINTREPORT_CHANNEL 15
#endif

#ifndef ETHERNET_CONTOUT1_CHANNEL
  #define ETHERNET_CONTOUT1_CHANNEL 16
#endif

#ifndef ETHERNET_CONTOUT2_CHANNEL
  #define ETHERNET_CONTOUT2_CHANNEL 17
#endif

#define USBFILE_PRINTREPORT_CHANNEL 9999

#define ETHERNET_UNITRECORD_PORT   ":2500"
#define ETHERNET_CONTOUT1_PORT     ":2501"
#define ETHERNET_CONTOUT2_PORT     ":2502"
#define ETHERNET_PRINTER_PORT      ":9100"
#define ETHERNET_PRINTREPORT_PORT  ":9101"


#ifndef SCALE_CHANNEL     //Channel Reference- used only as base #
  #define SCALE_CHANNEL  10 
#endif  

#ifndef SCALE_CHANNEL1
  #define SCALE_CHANNEL1  11 
#endif 

#ifndef SCALE_CHANNEL2
  #define SCALE_CHANNEL2  12 
#endif 

//#ifndef SCALE_NUMBER      //Number Reference- used only as base #
//  #define SCALE_NUMBER   0 
//#endif

#ifndef SCALE_NUMBER1
  #define SCALE_NUMBER1   1 
#endif

#ifndef SCALE_NUMBER2
  #define SCALE_NUMBER2   2 
#endif
...
//------------------\\
{
      ...
      decimal   Overfill1%6.2;
      decimal   Overfill2%6.2;          
      decimal   Cutoff1%6.2;
      decimal   Cutoff2%6.2;
      decimal   BulkCutoff1%6.2;
      decimal   BulkCutoff2%6.2;
      decimal   DFTime%5.2;
      ...
      decimal   MinFeeding1%6.2;       //%
      decimal   MaxFeeding1%6.2;       //%
      decimal   MinFeeding2%6.2;     //%
      decimal   MaxFeeding2%6.2;     //%
      int       StreamRegConfig;       //enum list Low-High-Auto
      decimal   ActuatorPos1%6.2;     //% 
      decimal   ActuatorPos2%6.2;     //% 
      ...
      decimal   FlexT2%5.2;
      bool      CountAll;
      decimal   ProdUnderWeightCnt_1%10.0;
      decimal   ProdUnderWeightCnt_2%10.0;
      decimal   ProdOverWeightCnt_1%10.0;
      decimal   ProdOverWeightCnt_2%10.0;
      decimal   ProdTotalUnitsCnt_1%10.0;
      decimal   ProdTotalUnitsCnt_2%10.0;
      double    ProdTotalWeight_1;
      double    ProdTotalWeight_2;
      int       SlowFillFactor;
      string    CPSmFastAvgPoints1[128];
      string    CPSmSlowAvgPoints1[128];
      string    CPSmSlowItpPoints1[512];
      float     CPSmSwitchTime1;
      string    CPFlFastPoints1[128];
      string    CPFlDeviationPoints1[256];
      string    CPSmFastAvgPoints2[128];
      string    CPSmSlowAvgPoints2[128];
      string    CPSmSlowItpPoints2[512];
      float     CPSmSwitchTime2;
      string    CPFlFastPoints2[128];
      string    CPFlDeviationPoints2[256];
...
decimal MinAccepted%6.2;
decimal MaxAccepted%6.2;
decimal Residue%6.2;
decimal StdDev%6.4;
decimal Avg%6.2;
...
bool    SuspendLearn_InitialImpact;
}
FeederSt Feeder[11];
...
nov double  SumWeight1NOV,AvgWeight1NOV,SqareSumWeight1NOV,StdDev1NOV;
nov double  SumWeight2NOV,AvgWeight2NOV,SqareSumWeight2NOV,StdDev2NOV;
nov string  SqareSumWeightStringNOV[2][256],SumWeightStringNOV[2][256];
nov int     NbrCheckedNOV[2];
nov decimal ProdUnderWeightCntNOV%10.0[2],ProdOverWeightCntNOV%10.0[2],ProdAcceptUnitCntNOV%10.0[2],ProdTotalUnitsCntNOV%10.0[2];
nov decimal ProdStdDevNOV%6.4[2],ProdAvgWeightNOV%6.2[2];
nov double  ProdTotalWeight1NOV,ProdTotalWeight2NOV;
nov decimal ProdLastCheckedNOV%7.3[2], ProdLastWeightNOV%7.3[2];


//LifeTime Production Data
nov decimal ProdTotalWeighmentsLTNOV%10.0,ProdTotalUnitsLTNOV%10.0;
...
nov decimal   MinFeedingMemNOV%6.2[2],MaxFeedingMemNOV%6.2[2],TargetWeightMemNOV%6.2[2];
...
nov BatchStruct BatchNov[WPdbSIZE];

nov decimal      WeighPerUnitAccNOV%5.0;
nov bool         WeighPerUnitDoneNOV,WeighmentInPanNOV[2];
...
#define PragmaLanguages   "English,0,[GB,US,CA];Arabic,0,[GB,US,CA];Chinese,0,[GB,US,CA];"\
                          ...
                          "Slovak,0,[GB,US,CA];Spanish,0,ES;Thai,0,[GB,US,CA];Turkish,0,[GB,US,CA];SpareLanguage,0,[GB,US,CA];" 
                          
#pragma (languages, PragmaLanguages);
string PRAGMALanguage[MAX_LANG][24]; bool LangTranslated[MAX_LANG];
string TransLang[MAX_LANG][24]; 

//To avoid the Yellow screen on power-up if Scale is not OK
#pragma (wait_for_scale, "0")

//Production Report & Auto Page Related Data
double  SumWeight1,AvgWeight1,SqareSumWeight1,StdDev1;
double  SumWeight2,AvgWeight2,SqareSumWeight2,StdDev2;
string  SqareSumWeightString[2][256],SumWeightString[2][256];
int     NbrChecked[2];
decimal ProdUnderWeightCnt%10.0[2],ProdOverWeightCnt%10.0[2],ProdAcceptUnitCnt%10.0[2],ProdTotalUnitsCnt%10.0[2];
decimal ProdStdDev%6.4[2],ProdAvgWeight%6.2[2];
double  ProdTotalWeight1,ProdTotalWeight2;
decimal ProdLastChecked%7.3[2], ProdLastWeight%7.3[2];

//LifeTime Production Data
decimal ProdTotalWeighmentsLT%10.0,ProdTotalUnitsLT%10.0;
double  ProdTotalWeightLT;

//Production Report & Auto Page Related Data
float        CheckedWeight[2], CorrWeight[2];
bool         ProdUnderWeightFlag[2],ProdOverWeightFlag[2], ProdDataReadyPLC;
int          ProdDataReadyNo;

decimal      ProdBulkDuration%6.2[2], ProdDribbleDuration%6.2[2],ProdCycleDuration%6.2[2], ProdCycleUnitsPerMin%6.2;
float        FastCutoff[2],SlowCutoff[2];
decimal      WeighPerUnitAcc%5.0;
bool         WeighPerUnitDone,WeighmentInPan[2];
bool         TopUp[2];
...
int          SMFilterSize;
bool         InitDone,ContinueUnitFlag,DischargedCall,IsCleanoutDoneFlag[2];

bool         fscan,AccessOK; // First scan indicator
bool         FirstScanBT,FirstScanCheckLearn;
bool         Refresh_BTInit,VarInitOffFlag,OutResetFlag,DischargingMem;
bool         ServiceModeEntered, BypassSMRestart, Simul;
bool         StartHMI, StopHMI, OffHMI, DischargeHMI1,DischargeHMI2, HopperHMI, ResetDataHMI,ResetBatchHMI, ResetLifetimeDataHMI, CheckWeightHMI, AutoTuneHMI, AutoMode; 

string       OldMsg[255], StepMsg[2][255];
...
int          mode_step, mode_step_MEM, Fault_Status, Fault_Status_MEM, Hopper_State, AutoTune_State, AutoTune_StateMEM, 
             Discharge_StateImage,Discharge_State[2], CheckWeight_State, CheckWeight_StateMem, Language_Cnt, ToggleBT_Cnt,NumberOfWPMem, Lang_No_Mem,
             SystemCleanMem1,SystemClean1,SystemCleanMem2,SystemClean2;
int          MemRecipeIO,RecipeFromInputs;
bool         LoadWPFromInputsFlag, ChangeWPInStopMode;             
             
int          PRIORITY_MDisplay, High_Msg_MDisplay,Fault_NoOut[2],Fault_NoOutStp[2],Fault_NoOutFault[2],Fault_NoOutput;
string       Msg_MDisplay[255], StepMsg_MDisplay[255], MessageOut[2][255],Fault_TxtOutput[255],MessageOutStp[2][255],MessageOutFault[2][255];
bool         OffRequest, Permission;
bool         CleanButton1, CleanButton2; //SADM2
bool         FaultLightSetFlag, Message_DisplayFlag[2], CurModeSetFlag,HopperStateFlag,AutoTuneStateFlag, DischargeStateFlag, FaultResetFlag, 
...
decimal   MinFeedingMem%6.2[2],MaxFeedingMem%6.2[2],TargetWeightMem%6.2[2];
decimal   NominalWeightMem%6.2,OverfillMem1%6.2,OverfillMem2%6.2, ActuatorPosMem1%6.2, ActuatorPosMem2%6.2;
int       SlowFilterSizeMem,FastFilterSizeMem,FlashFilterSizeMem,StdFilterSizeMem,StreamRegConfigMem;
float     iQFlSwitchWeightMem,iQFlReduceOutputMem,iQFlControlTimeMem,req_slow_fill_timeMem,iQDeadTimeRead[2];
int       RecipeMem,FeedAlgoTypeMem;
bool      RecipeButton,GotoPageButton[11];
bool      FirstScanScreenDone[LAST_SCREEN];
bool      VibrationEnabled;
decimal   BulkCO1%6.2, BulkCO2%6.2;
float     Open2ndDoorLastPart = 0.4;
...
int OUT_Command, OUT_Parameter, OUT_Value_DoubleWord[64], OUT_SeqMaster[2], OUT_SeqModeManager, OUT_FaultGen
    OUT_FaultBlock, OUT_FaultStopping, OUT_FaultWarn, OUT_LiveWeight_MSB[2], OUT_LiveWeight_LSB[2];
int OUT_dp,OUT_ScaleStatus, OUT_WeighProgram,OUT_WeightUnit;

int IN_Command, IN_Parameter,IN_Handshake;
int IN_CommandMem,IN_ParameterMem;
float IN_Value,IN_ValueMem;
bool  OUT_Handshake;
bool  BatchEndViaComm;
...
bool WriteUSBStick = OFF, Synchro_Test;
string PrtPortPara[25],TicketInfo[256];
...
string       ScaleDriver[MaxScales][32], Detail[32];
string       iQSimFeedProg[256],iQSimDischProg[256],iQFeedAlgorithmName[2][24];
...
string       HiResColor[20],MessageText[2][100],MessageColor[2][20];
...
int          MaxNumbOfIO;    
int  TEST_BUTTON[10], TEST_ID[12], TEST_VALUE[12], TEST_CHECK[12], TEST_POINTER[12], TEST_STATE[12], TEST_SELECT[12], Test_No, Test_PW;
...
bool AOUTActive[4],AINActive[2],FeederBWithAout[2];
string AOUTType[4][1],AINType[2][1],AOUTName[5][10];
int AOUTMode[4];
int AOUTModule[4];
float ActuatorPosition[2], ActuatorLivePos[2];

//Temp DORJ2 Test vars
bool Continuous_Output, AutoPage_FS, RT_AutoPage;
bool LowLevel[2], ResetRegistryKey; 
string lcaseAddress[24], lcaseAddress2[24];
int MinuteInAccess;
...
int iQFeedSettledCnt[2];
int iQSmartLearnNbr[2],iQFlashLearnNbr[2];
// IO Write test
int  MaxArrayFlexIODbase;
...
bool IO_Enable[IO_ARRAY],IO_Input[IO_ARRAY],IO_State[IO_ARRAY],IO_TestState[IO_ARRAY], IO_Force[IO_ARRAY], Recipe_Grayout[49], MP_Grayout[37], MC_Grayout[37], 
...
float   ScanMin;
float   ScanTotal;
bool    OperationTaskHeartBeat,FillingTaskHeartBeat,BypassTasksMonitor;
...
int     SeqMaster,Seq2Master, SeqSpout, SeqWeighPan,Seq2WeighPan, SeqFeeder,Seq2Feeder,SeqFeedSTD,Seq2FeedSTD,SeqManDischarge,Seq2ManDischarge,SeqKicker;
int     printTime,PrintEnd,PrintBegin;

//From Back_Task.c
//Scale dependant Variables
enum {Scale1=0,Scale2=1,ScaleBoth=3}
int Scale1Adm,Scale2Adm;
enum {Scale1CmpBulk=1,Scale1CmpDribble=2,Scale1CmpCatchGate=3,
      Scale1CmpAout1=4,Scale1CmpAout2=5,
      Scale2CmpBulk=9,Scale2CmpDribble=10,Scale2CmpCatchGate=11,
      Scale2CmpAout1=12,Scale2CmpAout2=13}
bool CmpBulkActive[2],CmpDribbleActive[2],CmpCatchGateActive[2], CmpAnalogActive[2];      
bool IsSettled[2],IsZero[2];
int ZeroFreqCnt[2], SampleFreqCnt[2];
bool EmptyHopper,CheckAll,CheckOnce[2],SlowCycleMem[2];
bool SlowCycleOccured[2],IsCleanoutDone[2],CleanoutWeighment[2],DischReqInputOn;
int ZeroFreqVal,Hopper_StateMEM,Discharge_StateImageMEM,Current_ScreenMEM,Current_ScreenMEM2;
...
bool BulkOnTime,WeightSampled[2],LastCheckedWeight[2];

bool WeighmentAborted[2],FeedInProcess[2],HoldCycleMem, HoldCycleTgl[2], CycleInHoldMem[2], DisplayAlarmDelay[2],BlockMessage[2],DisplayFault[2];
...
string STR_BATCH_MAX_ACCEPTED[20];
string STR_BATCH_MIN_ACCEPTED[20];
...
string STR_ID_TOTAL_WEIGHT_W_UNIT[20];
...
string STR_BatchMinAccepted[10];
string STR_BatchUnitsDone[10];
string STR_BatchMaxAccepted[10];
...
string STR_ProdTotalWeighmentsLT[10];
string STR_ProdTotalUnitsCnt_Scale1[10];
string STR_ProdTotalUnitsCnt_Scale2[10];
string STR_ProdTotalUnitsCnt_ScaleBoth[10];
string STR_ProdTotalUnitsLT[10];
string STR_ProdAcceptUnitCnt_Scale1[10];
string STR_ProdAcceptUnitCnt_Scale2[10];
string STR_ProdAcceptUnitCnt_ScaleBoth[10];
string STR_ProdTotalWeightLT[17];
string STR_ProdUnderWeightCnt_Scale1[10];
string STR_ProdUnderWeightCnt_Scale2[10];
string STR_ProdUnderWeightCnt_ScaleBoth[10];
string STR_ProdOverWeightCnt_Scale1[10];
string STR_ProdOverWeightCnt_Scale2[10];
string STR_ProdOverWeightCnt_ScaleBoth[10];
string STR_ProdTotalWeight_Scale1[14];
string STR_ProdTotalWeight_Scale2[14];
string STR_ProdTotalWeight_ScaleBoth[14];


// Flash Feed
string      CPSmFastAvgPointsMem[2][128],CPSmSlowAvgPointsMem[2][128]CPSmSlowItpPointsMem[2][512];
string      CPFlFastPointsMem[2][128],CPFlDeviationPointsMem[2][256];
float       CPSmSwitchTimeMem[2],CPSmSwitchTimePct=0.85;
bool        LearniQSmartParam[2],LearniQSmartParamMem[2],LearniQFlashParam[2],iQFeedDone[2],LearniQSmartToSave[2],LearniQFlashToSave[2];
bool        InitiQSmart[2],InitiQFlash[2],iQOutputEnabled[2],InitCPSmSwitchTime[2];  
bool        SaveiQFeedParamFlag[2];
int         NbrOf1stLearned[2];
...
//iQReadEventStats Production data
string iQStatData_String1[2][32], 
       iQStatData_String2[2][32], 
       iQStatData_String3[2][32], 
       iQStatData_String4[2][32], 
       iQStatData_String5[2][32], 
       iQStatData_String6[2][32], 
       iQStatData_String7[2][32],  
       iQStatData_String8[2][32],  
       iQStatData_String9[2][32]; 

//Smart Feed Data
float stat_controlled_fast_time[2],
      stat_controlled_slow_time[2], 
      stat_stable_fast_flow[2], 
      stat_stable_slow_flow[2], 
      stat_fast_flow_fluctuation[2], 
      stat_slow_flow_fluctuation[2];
...
bool ShowDuplex = OFF,ScaleSwitched,AutoPageView,ReportOnCumulated=ON;
bool Normal = ON;
string Weight_1and2[40];
string  KeyboardString[255],RecipeRowString[7][40];
decimal RecipeRowValue%6.2[2][7];
int ShownScale,WeigherNbr,taskid_keyboard,taskid_contout;
bool KeyboardReq,KeyboardDone;
string KeyboardPrompt[40], KeyboardVal[10];
bool RecipeDualInput[49];
int RecipeDualInIndex[49];
bool IONamesRefreshed,Scale1ON,Scale2ON;
int  WeigherNoDisch=1,EnabledScaleMEM=-1,StatDataNo,ContPortMem=-1;
bool UnitCompleted,StopForUnit[2];
...
float ContOutIntervalTime;
bool O_CatchGateMem,O_CatchGateMem2,CatchGateAlreadyOpen,CatchGateAlreadyOpen2,O_DischDoorMem,O_DischDoorMem2,DischDoorStillClose,DischDoorStillClose2;
int     MouseMem = -1;
...
//                                           widget_text (RECIPE_SEARCH_ID, concat(space(1),lsub(WP.Name),space(10), "4" , space(1), TX_FSLASH, space(1), "4", space(1)) );
                            ...
                            break;
                default : 
                          if(value>RECIPE_INPUT_FIRST && value<RECIPE_INPUT_LAST)
                           if(RecipeDualInput[value-RECIPE_INPUT_FIRST])
                             {
                              if (Permission)
                               {
                                Wnbr = Weigher1or2Popup(OFF);
                                if(Wnbr == 1)
                                 WeigherNbr = 1;
                                if(Wnbr == 2)
                                 WeigherNbr = 2;
                              
                                 KeyboardPrompt = RecipeRowString[RecipeDualInIndex[value - RECIPE_INPUT_FIRST]]; 
                                 KeyboardVal = adj(RecipeRowValue[WeigherNbr-1][RecipeDualInIndex[value - RECIPE_INPUT_FIRST]],6,2);
                                 KeyboardReq = ON;
        ...
        case EVENT_TYPE_TEXT_ENTERED:   
           ...
           if (Permission)
           {  
              RefreshRecipeInputEntry(Current_Screen, value,args[0],0);
              SaveWP2Db();
           }   
           RefreshWPScreen (Current_Screen);
      ...
      if(KeyboardReq && KeyboardDone)
       {
        if(KeyboardString!="")
        {
         RefreshRecipeInputEntry(Current_Screen, value,KeyboardString,WeigherNbr);
         SaveWP2Db();
         RefreshWPScreen (Current_Screen);
        } 
        KeyboardReq = OFF;
                                ...
                                case SCREEN_IOCFG_IN1 :
                                 ...
                                 case SCREEN_IOCFG_OUT1 :
                                  ...
                                  case SCREEN_IOCFG_OUT2 :
                                         ...
                                         break;
                                  case SCREEN_IOCFG_OUT3 :
                                           SelectAndWaitPage(PAGE_IO_OUTPUT_4);
                                           Current_Screen = SCREEN_IOCFG_OUT4;                                             
                                         break;               
                              }
                              if (!LangRefreshed[Current_Screen]) RefreshLanguage(Current_Screen);
                              RefreshIOConfigScreen (Current_Screen); 
           ...
           if (Permission)
           {
             switch (Current_Screen)
             {
                ...
                case SCREEN_IOCFG_OUT3 :  
                           ...
                           break;
                case SCREEN_IOCFG_OUT4 :  
                             IOdb.No = (value)-IO_INPUTBOX_MARKER+72;
                           break;                                                                                                                                                                    
             }
           ...
           if (Permission)
           {
             switch (Current_Screen)
             {
                ...
                case SCREEN_IOCFG_OUT3 :  
                           ...
                           break; 
               case SCREEN_IOCFG_OUT4 :  
                             IOdb.No = (value)-IO_CHECKBOX_MARKER+72;
                             RefreshIOGrayOut(Current_Screen,12,72);
                           break;              
             }
      ...
      switch (type)
      {
        ...
        case EVENT_TYPE_BUTTON_PRESS:   
             ...
             {                                 
                ...
                case NAV_FCT_BUTTON_6 : 
                              if (Current_Screen==SCREEN_IOTEST_IN1 || Current_Screen==SCREEN_IOTEST_IN2 || Current_Screen==SCREEN_IOTEST_IN3 || Current_Screen==SCREEN_ANALOGTEST)  
                              Current_Screen=SCREEN_IOTEST_IN1;
                              if (Current_Screen==SCREEN_IOTEST_OUT1 || Current_Screen==SCREEN_IOTEST_OUT2 || Current_Screen==SCREEN_IOTEST_OUT3 || Current_Screen==SCREEN_IOTEST_OUT4)
                ...
                case NAV_FCT_BUTTON_10 :
                              switch (Current_Screen)
                              {
                                case SCREEN_IOTEST_IN1 :
                                 ...
                                 case SCREEN_IOTEST_OUT3 :
                                           SelectAndWaitPage(PAGE_IO_OUTPUTTEST_4);
                                           Current_Screen = SCREEN_IOTEST_OUT4;                                          
                                           RefreshIOGrayOut(Current_Screen, 12,72);                                                                                   
                                         break;                
                              }
                              if (!LangRefreshed[Current_Screen]) RefreshLanguage(Current_Screen);
           ...
           if (Permission)
           {
             switch (Current_Screen)
             {
                ...
                case SCREEN_IOTEST_OUT3 :  
                           ...
                           break;
                case SCREEN_IOTEST_OUT4 :  
                             object_id = (value)-IO_CHECKBOX_MARKER+72;
                             IO_Force[object_id] = (!Arg2Bool(args[0])&& !LT_GEN);
                           break;             
...
string  args[4][255];
 ...
 { 
  ...
  if(!FirstScanScreenDone[Current_Screen]) SwitchReport2ndRow();
  if (!LangRefreshed[Current_Screen]) RefreshLanguage(Current_Screen);  
  if (!ButtonDisplayRefreshed[Current_Screen])  ButtonDisplayControl(Current_Screen);
  RefreshReportScreen(Current_Screen); 
  widget_enable     (NAV_FCT_BUTTON_7, ON);
...
//                              break; 
  ...
  //                            break;
                  ...
                  case GOTO_FCT_BUTTON_3 : //SADM2
                              if (Permission)
                               {
                               if (!SystemClean1 && !SystemClean2)
                                {
                                 Wnbr = Weigher1or2Popup(OFF);
                                 if(Wnbr == 1)
                                 WeigherNbr = 1;
                                ...
                                if(WeigherNbr == 1)
                                 {
                                  ...
                                  if (!SystemClean1)
                                    {
                                     YesNoPopupMessage(lsub(TX_CLEANSYSTEM), SCREEN_GOTO, NIL, NO_Db);
                                    }
                                    else
                                    SystemClean1 = OFF;
                                    break;
...
int      err;
bool     SMlocked;
int AutoSwitchTime,AutoSwitchPrev;
...
if (!ButtonDisplayRefreshed[Current_Screen])  ButtonDisplayControl(Current_Screen);
  ...
  {  
    ...
    if ( eventreceived() )
    {
      ...
      if(SMlocked)
       ...
       switch (type)
       {
        ...
        case EVENT_TYPE_BUTTON_PRESS:
             ...
             { 
               ...
               case ACTIVE_RECIPE_ID :
                ...
                case NAV_FCT_BUTTON_1 :
                            AutoSwitchPrev = systemtimer_ms();
                            AutoPageView =  AutoPageSwitchView(AutoPageView); 
                            RefreshLanguage(SCREEN_AUTOMATIC);
                            AutoSwitchTime = systemtimer_ms()- AutoSwitchPrev;
        ...
        case EVENT_TYPE_SCALE_BUTTON:                      
                       ...
                       if (value == SCALE_BUTTON_SCALE)
                       {
                          ShownScale = SwitchScale(OFF);                                           
                       }
           break;
        ...
        case EVENT_TYPE_CONNECTION_ESTABLISHED:   
                               ...
                               Message_DisplayFlag[Scale1] = Execute;
                               Message_DisplayFlag[Scale2] = Execute;
      ...
      {
       if(IsCleanoutDone[Scale1])
        {
         IsCleanoutDoneFlag[Scale1] = ON;
         YesNoPopupMessage(concat("#1 ",lsub(TX_CLEANOUTCOMPLETED),"?"), SCREEN_AUTOMATIC, NIL, NO_Db);
         IsCleanoutDoneFlag[Scale1] = OFF;
        }
         
       else if(IsCleanoutDone[Scale2])
        {
         IsCleanoutDoneFlag[Scale2] = ON;
         YesNoPopupMessage(concat("#2 ",lsub(TX_CLEANOUTCOMPLETED),"?"), SCREEN_AUTOMATIC, NIL, NO_Db);
         IsCleanoutDoneFlag[Scale2] = OFF;
        }
      ...
      if(SampledInPause)
       { 
        widget_text(HIRESWEIGHT,concat(adj(NetWeightHiRes[Scale1],6,dp+1),space(1),adj(NetWeightHiRes[Scale2],6,dp+1),space(1),WP.Unit,space(1)));
       }
      else   
       { 
      ...
      Weight_1and2 = concat("W1 ",adj(NetWeight[Scale1],6,DP[Scale1]),WP.Unit,"\n","W2 ",adj(NetWeight[Scale2],6,DP[Scale2]),WP.Unit);
      widget_text(DUPLEX_WEIGHT,Weight_1and2);
...
bool err;
  ...
  OffRequest    = OFF;
  CleanButton1  = OFF; //SADM2
  CleanButton2   = OFF; //SADM2
  mode_step     = 0;
  Fault_Status  = 0;
  tmpBTN1  = "";
  tmpBTN2  = "";
...
// Start background task
  ...
  //setpriority (taskid_operation, 3);
   ...
   //setpriority (taskid_print, 5);
    ...
    /* Start Keyboard task */
   taskid_keyboard = starttask (KeyboardTask); 
   //setpriority (taskid_keyboard, 7);  
   
   /* Start Continuous Output task */
   taskid_contout = starttask (ContOutTask); 
   //setpriority (taskid_contout, 9);  
