...
if (Scrn_No==SCREEN_RECIPE1 || Scrn_No==SCREEN_RECIPE2 || Scrn_No==SCREEN_RECIPE3 || Scrn_No==SCREEN_RECIPE4) 
 ...
 begindraw();
    switch (Scrn_No_Temp)
    {
      ...
      case SCREEN_AUTOMATIC :
        ...
        widget_enable(NAV_FCT_BUTTON_8, (mode_step != stp_OFF_MODE && SeqModeManager != stp_STOP_MODE) 
                                         && (IO_Enable[O_HopFillReq] || !Input_State(I_LowLevelSensor1) || !Input_State(I_LowLevelSensor2)));
      ...
      case SCREEN_GOTO :
        ...
        else
        {
           ...
           widget_enable     (GOTO_FCT_BUTTON_3,  (mode_step == stp_OFF_MODE));
      ...
      case SCREEN_BATCH : //
           ...
           BatchEditAllowed = (mode_step == stp_OFF_MODE || mode_step == stp_STOP_MODE);

           widget_enable     (REPORT_BATCHNAME_INPUT,     BatchEditAllowed);
           widget_enable     (REPORT_BATCHPRESET_INPUT,   BatchEditAllowed);
 ...
 if(!FirstScanScreenDone[Current_Screen]) FirstScanScreenDone[Current_Screen] = ON;
 enddraw();
} 
