...
void KeyboardTask(void)
{
 ...
 for(;;)
 {
  if(KeyboardReq && !KeyboardDone)
  {
   KeyboardString = requestinput (KEYBOARD_MODE_NUMERIC, KeyboardPrompt, KeyboardVal, INPUT_FORMAT_DECIMAL, 8, 2);
   KeyboardDone = ON;
  }
  delay(0.5);
 }   
...
bool AutoPageSwitchView(bool State)
{
 int i;
  
 for(i=AUTO_ROW11_ID;i<= AUTO_ROW24_UNIT;i++)
  {
   if(!State)
   {
    if(i<AUTO_ROW21_ID)
...
string image[64];
 ...
 if (enable && (!GotoPageButton[button_nbr-GOTO_PAGE_BUTTON_1+1] || !FirstScanScreenDone[Current_Screen]))
  {
...
/* Check if OIML setting hase changed*/
void CheckOIML(void)
{  
 if(MC.OIML)
  {
...
void SaveProdVars(void)
{
 ...
 for(i=0;i<=1;i++)
 {
  ...
  SumWeight1NOV            = SumWeight1;
  SumWeight2NOV            = SumWeight2;
  AvgWeight1NOV            = AvgWeight1;
  AvgWeight2NOV            = AvgWeight2;
  SqareSumWeight1NOV       = SqareSumWeight1;
  SqareSumWeight2NOV       = SqareSumWeight2;
  StdDev1NOV               = StdDev1;
  StdDev2NOV               = StdDev1;
  ProdTotalWeight1NOV      = ProdTotalWeight1;
  ProdTotalWeight2NOV      = ProdTotalWeight2;
  ProdTotalWeighmentsLTNOV   = ProdTotalWeighmentsLT;
  ProdTotalUnitsLTNOV        = ProdTotalUnitsLT;
  ProdTotalWeightLTNOV       = ProdTotalWeightLT;
...
void LoadProdVars(void)
{
 ...
 for(i=0;i<=1;i++)
 {
  ...
  SumWeight1            = SumWeight1NOV;
  SumWeight2            = SumWeight2NOV;
  AvgWeight1            = AvgWeight1NOV;
  AvgWeight2            = AvgWeight2NOV;
  SqareSumWeight1       = SqareSumWeight1NOV;
  SqareSumWeight2       = SqareSumWeight2NOV;
  StdDev1               = StdDev1NOV;
  StdDev2               = StdDev1NOV;
  ProdTotalWeight1      = ProdTotalWeight1NOV;
  ProdTotalWeight2      = ProdTotalWeight2NOV;
  ProdTotalWeighmentsLT   = ProdTotalWeighmentsLTNOV;
  ProdTotalUnitsLT        = ProdTotalUnitsLTNOV;
  ProdTotalWeightLT       = ProdTotalWeightLTNOV;
...
void WeighPerUnitLoad(void)
{
  ...
  WeighmentInPan[Scale1] = WeighmentInPanNOV[Scale1];
  WeighmentInPan[Scale2] = WeighmentInPanNOV[Scale2];
}
...
void WeighPerUnitSave(void)
{
 int scanmem,scanduration;
  ...
  WeighmentInPanNOV[Scale1] = WeighmentInPan[Scale1];
  WeighmentInPanNOV[Scale2] = WeighmentInPan[Scale2];
...
{
MinFeedingMem[Scale1]   = MinFeedingMemNOV[Scale1];
MaxFeedingMem[Scale1]   = MaxFeedingMemNOV[Scale1];
MinFeedingMem[Scale2]   = MinFeedingMemNOV[Scale2];
MaxFeedingMem[Scale2]   = MaxFeedingMemNOV[Scale2];
...
{
MinFeedingMemNOV[Scale1]   = MinFeedingMem[Scale1];
MaxFeedingMemNOV[Scale1]   = MaxFeedingMem[Scale1];
MinFeedingMemNOV[Scale2]   = MinFeedingMem[Scale2];
MaxFeedingMemNOV[Scale2]   = MaxFeedingMem[Scale2];
...
int ScaleNo;
 ...
 {
  NbrChecked[ScaleNo-1] = 0; 
  SqareSumWeightString[ScaleNo-1] = "0.0";
  SumWeightString[ScaleNo-1] = "0.0";
 
  if(!OnlyAvgAndStdDev)
  {
   CheckedWeight[ScaleNo-1] = 0;
   CorrWeight[ScaleNo-1] = 0;
   ProdUnderWeightCnt[ScaleNo-1] = 0;
   ProdOverWeightCnt[ScaleNo-1] = 0;
   ProdAcceptUnitCnt[ScaleNo-1] = 0;
   ProdTotalUnitsCnt[ScaleNo-1] = 0;
   ProdLastChecked[ScaleNo-1] = 0;
   ProdLastWeight[ScaleNo-1] = 0;
   ProdLastSampled[ScaleNo-1] = "";
   ProdLastTolerance[ScaleNo-1] = "";
   ProdStdDev[ScaleNo-1] = 0;
   ProdAvgWeight[ScaleNo-1] = 0;
   ProdBulkDuration[ScaleNo-1] = 0;
   ProdDribbleDuration[ScaleNo-1] = 0;
   ProdCycleDuration[ScaleNo-1] = 0 ;
...
RecipeDualInput[2] = ON;RecipeDualInput[3] = ON;RecipeDualInput[4] = ON;
RecipeDualInput[6] = ON;RecipeDualInput[13] = ON;RecipeDualInput[14] = ON;RecipeDualInput[18] = ON;
RecipeDualInIndex[RECIPE_ROW2_INPUT - RECIPE_INPUT_FIRST] = 0;
RecipeDualInIndex[RECIPE_ROW3_INPUT - RECIPE_INPUT_FIRST] = 1;
RecipeDualInIndex[RECIPE_ROW4_INPUT - RECIPE_INPUT_FIRST] = 2;
RecipeDualInIndex[RECIPE_ROW6_INPUT - RECIPE_INPUT_FIRST] = 3;
RecipeDualInIndex[RECIPE_ROW13_INPUT - RECIPE_INPUT_FIRST] = 4;
RecipeDualInIndex[RECIPE_ROW14_INPUT - RECIPE_INPUT_FIRST] = 5;
RecipeDualInIndex[RECIPE_ROW18_INPUT - RECIPE_INPUT_FIRST] = 6;
mode_step=0;
StepMsg[Scale1] = "";
StepMsg[Scale2] = "";
CopyNovToLearnParam();
...
WeightDisplaySize=-1;
iQFeedSettledCnt[Scale1]=0;
iQFeedSettledCnt[Scale2]=0;
IO_Change = OFF;
DischReqInputOn = ON;
...
T_PRE[T_iQFeedTimeout2]=60.00;
T_PRE[T_InitAlgo]       = 1.0;
T_PRE[T_InitAlgo2]      = 1.0;
Scale1Adm=1;
Scale2Adm=2;
iQStatData_String1[Scale1] = "NO DATA1";
iQStatData_String2[Scale1] = "NO DATA2"; 
iQStatData_String3[Scale1] = "NO DATA3"; 
iQStatData_String4[Scale1] = "NO DATA4"; 
iQStatData_String5[Scale1] = "NO DATA5"; 
iQStatData_String6[Scale1] = "NO DATA6"; 
iQStatData_String7[Scale1] = "NO DATA7";  
iQStatData_String8[Scale1] = "NO DATA8";  
iQStatData_String9[Scale1] = "NO DATA9"; 
iQStatData_String1[Scale2] = "NO DATA";
iQStatData_String2[Scale2] = "NO DATA"; 
iQStatData_String3[Scale2] = "NO DATA"; 
iQStatData_String4[Scale2] = "NO DATA"; 
iQStatData_String5[Scale2] = "NO DATA"; 
iQStatData_String6[Scale2] = "NO DATA"; 
iQStatData_String7[Scale2] = "NO DATA";  
iQStatData_String8[Scale2] = "NO DATA";  
iQStatData_String9[Scale2] = "NO DATA"; 
...
LoadProdVars();
SqareSumWeight1 = double(SqareSumWeightString[Scale1]);
SumWeight1 = double(SumWeightString[Scale1]);
SqareSumWeight2 = double(SqareSumWeightString[Scale2]);
SumWeight2 = double(SumWeightString[Scale2]);
...
OpenConnection(1);
OpenConnection(2);
result[0]= iQReadCalFilterSize(SCALE_CHANNEL1, SCALE_NUMBER1,SMFilterSize);
result[1]= SetHiResolutionFactor(SCALE_CHANNEL1, SCALE_NUMBER1,HIRES_X10);
result[2]= SetHiResolutionFactor(SCALE_CHANNEL2, SCALE_NUMBER2,HIRES_X10);

popen (ETHERNET_UNITRECORD_CHANNEL, ETHERNET_UNITRECORD_PORT); //Open port for Unit Record "broadcast" 
popen (ETHERNET_PRINTREPORT_CHANNEL,ETHERNET_PRINTREPORT_PORT); //Open port for Print Report "broadcast"
popen (ETHERNET_CONTOUT1_CHANNEL, ETHERNET_CONTOUT1_PORT);
popen (ETHERNET_CONTOUT2_CHANNEL, ETHERNET_CONTOUT2_PORT);
...
int number[NUMBEROFOUTPUTS];

bool Checked;
    Checked = ON;
    k=0;
    for (i=NUMBEROFINPUTS+1;i<IO_ARRAY;i++)
    {
     if(IO_Enable[i])
     {
...
void SaveCP2WP(void)
{
  ...
  int i,len,ScaleNo;
   ...
   for(ScaleNo = Scale1; ScaleNo <= Scale2; ScaleNo++)
   {
      //Smart Fast Average Points  
        if(ScaleNo == Scale1) tempstr = WP.CPSmFastAvgPoints1;
        else                  tempstr = WP.CPSmFastAvgPoints2;
        i=0;
         do
          {
           len = length(tempstr);
           if (len > 63)
            {
             if(ScaleNo == Scale1)  WPdb.CPSmFastAvgPoints1[i] = left(tempstr,63);
             else                   WPdb.CPSmFastAvgPoints2[i] = left(tempstr,63);
             tempstr = right(tempstr,len-63);
            }
           else
            {
             if(ScaleNo == Scale1)  WPdb.CPSmFastAvgPoints1[i] = left(tempstr,len);
             else                   WPdb.CPSmFastAvgPoints2[i] = left(tempstr,len);
            }   
            i++;
          }
           while(len > 63);
      ...
      //Smart Slow Average Points  
        if(ScaleNo == Scale1)  tempstr = WP.CPSmSlowAvgPoints1;
        else                   tempstr = WP.CPSmSlowAvgPoints2;
        i=0;
         do
          {
           len = length(tempstr);
           if (len > 63)
            {
             if(ScaleNo == Scale1)  WPdb.CPSmSlowAvgPoints1[i] = left(tempstr,63);
             else                   WPdb.CPSmSlowAvgPoints2[i] = left(tempstr,63);
             tempstr = right(tempstr,len-63);
            }
           else
            {
             if(ScaleNo == Scale1)  WPdb.CPSmSlowAvgPoints1[i] = left(tempstr,len);
             else                   WPdb.CPSmSlowAvgPoints2[i] = left(tempstr,len);
            }   
            i++;
          }
           while(len > 63);
       ...
       //Smart Slow ITP Points    
        if(ScaleNo == Scale1) tempstr = WP.CPSmSlowItpPoints1;
        else                  tempstr = WP.CPSmSlowItpPoints2;
        i=0;
         do
          {
           len = length(tempstr);
           if (len > 63)
            {
             if(ScaleNo == Scale1)  WPdb.CPSmSlowItpPoints1[i] = left(tempstr,63);
             else                   WPdb.CPSmSlowItpPoints2[i] = left(tempstr,63);
             tempstr = right(tempstr,len-63);
            }
           else
            {
             if(ScaleNo == Scale1) WPdb.CPSmSlowItpPoints1[i] = left(tempstr,len);
             else                  WPdb.CPSmSlowItpPoints2[i] = left(tempstr,len);
            }   
            i++;
          }
           while(len > 63);
       ...
       //Smart Switch Time
        if(ScaleNo == Scale1)  WPdb.CPSmSwitchTime1    = WP.CPSmSwitchTime1;
        else                   WPdb.CPSmSwitchTime2    = WP.CPSmSwitchTime2;
        

      //Flash Fast Points 
       if(ScaleNo == Scale1)  tempstr = WP.CPFlFastPoints1;
       else                   tempstr = WP.CPFlFastPoints2;
       i=0;
         do
          {
           len = length(tempstr);
           if (len > 63)
            {
             if(ScaleNo == Scale1)  WPdb.CPFlFastPoints1[i] = left(tempstr,63);
             else                   WPdb.CPFlFastPoints2[i] = left(tempstr,63);
             tempstr = right(tempstr,len-63);
            }
           else
            {
             if(ScaleNo == Scale1)  WPdb.CPFlFastPoints1[i] = left(tempstr,len);
             else                   WPdb.CPFlFastPoints2[i] = left(tempstr,len);
            }   
            i++;
          }
           while(len > 63);
      ...
      //Flash Deviation Points    
       if(ScaleNo == Scale1)  tempstr = WP.CPFlDeviationPoints1;
       else                   tempstr = WP.CPFlDeviationPoints2;
       i=0;
         do
          {
           len = length(tempstr);
           if (len > 63)
            {
             if(ScaleNo == Scale1)  WPdb.CPFlDeviationPoints1[i] = left(tempstr,63);
             else                   WPdb.CPFlDeviationPoints2[i] = left(tempstr,63);
             tempstr = right(tempstr,len-63);
            }
           else
            {
             if(ScaleNo == Scale1)  WPdb.CPFlDeviationPoints1[i] = left(tempstr,len);
             else                   WPdb.CPFlDeviationPoints2[i] = left(tempstr,len);
            }   
            i++;
          }
           while(len > 63);
   ...
   for(ScaleNo = Scale1; ScaleNo <= Scale2; ScaleNo++)
   {
      //Smart Fast Average Points  
        ...
        i=0;
         do
          {
           if(ScaleNo == Scale1) len = length(WPdb.CPSmFastAvgPoints1[i]);
           else                  len = length(WPdb.CPSmFastAvgPoints2[i]);
           if (len > 0)
            {
             if(ScaleNo == Scale1) tempstr = concat (tempstr,WPdb.CPSmFastAvgPoints1[i]);
             else                  tempstr = concat (tempstr,WPdb.CPSmFastAvgPoints2[i]);
             i++;
            }
           if(i>=2) break;
          }
        while(len > 0);
        if(ScaleNo == Scale1)  WP.CPSmFastAvgPoints1 = tempstr;
        else                   WP.CPSmFastAvgPoints2 = tempstr;
      ...
      tempstr = "";
        i=0;
         do
          {
           if(ScaleNo == Scale1) len = length(WPdb.CPSmSlowAvgPoints1[i]);
           else                  len = length(WPdb.CPSmSlowAvgPoints2[i]);
           if (len > 0)
            {
             if(ScaleNo == Scale1)  tempstr = concat (tempstr,WPdb.CPSmSlowAvgPoints1[i]);
             else                   tempstr = concat (tempstr,WPdb.CPSmSlowAvgPoints2[i]);
             i++;
            }
           if(i>=2) break;
          }
        while(len > 0);
        if(ScaleNo == Scale1)  WP.CPSmSlowAvgPoints1 = tempstr;
        else                   WP.CPSmSlowAvgPoints2 = tempstr;  
       ...
       tempstr = "";
        i=0;
         do
          {
           if(ScaleNo == Scale1)  len = length(WPdb.CPSmSlowItpPoints1[i]);
           else                   len = length(WPdb.CPSmSlowItpPoints2[i]);
           if (len > 0)
            {
             if(ScaleNo == Scale1)  tempstr = concat (tempstr,WPdb.CPSmSlowItpPoints1[i]);
             else                   tempstr = concat (tempstr,WPdb.CPSmSlowItpPoints2[i]);
             i++;
            }
           if(i>=8) break;
          }
        while(len > 0);
        if(ScaleNo == Scale1)  WP.CPSmSlowItpPoints1 = tempstr;
        else                   WP.CPSmSlowItpPoints2 = tempstr;        
        
       
       //Smart Switch Time
        if(ScaleNo == Scale1) WP.CPSmSwitchTime1    = WPdb.CPSmSwitchTime1;
        else                  WP.CPSmSwitchTime2    = WPdb.CPSmSwitchTime2;
      ...
      tempstr = "";
        i=0;
         do
          {
           if(ScaleNo == Scale1) len = length(WPdb.CPFlFastPoints1[i]);
           else                  len = length(WPdb.CPFlFastPoints2[i]);
           if (len > 0)
            {
             if(ScaleNo == Scale1) tempstr = concat (tempstr,WPdb.CPFlFastPoints1[i]);
             else                  tempstr = concat (tempstr,WPdb.CPFlFastPoints2[i]);
             i++;
            }
           if(i>=2) break;
          }
        while(len > 0);
        if(ScaleNo == Scale1) WP.CPFlFastPoints1 = tempstr; 
        else                  WP.CPFlFastPoints2 = tempstr; 
      ...
      tempstr = "";
        i=0;
         do
          {
           if(ScaleNo == Scale1) len = length(WPdb.CPFlDeviationPoints1[i]);
           else                  len = length(WPdb.CPFlDeviationPoints2[i]);
           if (len > 0)
            {
             if(ScaleNo == Scale1) tempstr = concat (tempstr,WPdb.CPFlDeviationPoints1[i]);
             else                  tempstr = concat (tempstr,WPdb.CPFlDeviationPoints2[i]);
             i++;
            }
           if(i>=4) break;
          }
        while(len > 0);
        if(ScaleNo == Scale1) WP.CPFlDeviationPoints1 = tempstr; 
        else                  WP.CPFlDeviationPoints2 = tempstr;   
   }    
}
...
/*                                               */
/* ********************************************* */
bool CheckiQFeedLearn(int ScaleNo)
{
bool clearsmart,clearflash,learnsmart,learnflash,retval;
int min;
string    SmFastAvg[128],SmSlowAvg[128],SmSlowItp[512],FlFast[128],FlDeviation[256];
float     SmSwitchTime;

 if(ScaleNo == 1)
  {
    SmFastAvg     = WP.CPSmFastAvgPoints1;
    SmSlowAvg     = WP.CPSmSlowAvgPoints1;
    SmSlowItp     = WP.CPSmSlowItpPoints1;
    SmSwitchTime  = WP.CPSmSwitchTime1; 
    FlFast        = WP.CPFlFastPoints1;
    FlDeviation   = WP.CPFlDeviationPoints1;
  }
  
 if(ScaleNo == 2)
  {
    SmFastAvg     = WP.CPSmFastAvgPoints2;
    SmSlowAvg     = WP.CPSmSlowAvgPoints2;
    SmSlowItp     = WP.CPSmSlowItpPoints2;
    SmSwitchTime  = WP.CPSmSwitchTime2; 
    FlFast        = WP.CPFlFastPoints2;
    FlDeviation   = WP.CPFlDeviationPoints2;
  } 
 ...
 min = IQFEEDSETTLEDNBR;
    ...
    if(WP.FeedAlgoType == FeediQSmart)
     {
      if((CountChar(SmFastAvg,'.') >= min && CountChar(SmSlowAvg,'.') >= min && CountChar(SmSlowItp,':') >= min && SmSwitchTime!=0)
         && (!WP.TuneiQFeed || (WP.TuneiQFeed && FirstScanCheckLearn)))
       { 
        CPSmFastAvgPointsMem[ScaleNo-1] = SmFastAvg;
        CPSmSlowAvgPointsMem[ScaleNo-1] = SmSlowAvg;
        CPSmSlowItpPointsMem[ScaleNo-1] = SmSlowItp;
        CPSmSwitchTimeMem[ScaleNo-1]    = SmSwitchTime;
       }
      else
       {
        learnsmart=ON;
    ...
    if(clearsmart)
     {
      CPSmFastAvgPointsMem[ScaleNo-1] = "";
      CPSmSlowAvgPointsMem[ScaleNo-1] = "";
      CPSmSlowItpPointsMem[ScaleNo-1] = "";
    ...
    {
     if((CountChar(FlFast,'.') >= min && CountChar(FlDeviation,':') >= min) && !WP.TuneiQFeed)
      {
       CPFlFastPointsMem[ScaleNo-1]      = FlFast;
       CPFlDeviationPointsMem[ScaleNo-1] = FlDeviation;
      }
     else
      {
       learnflash=ON;
   ...
   if(clearflash) 
    {
     CPFlFastPointsMem[ScaleNo-1]      = "";
     CPFlDeviationPointsMem[ScaleNo-1] = "";
     clearflash=OFF;
    }
...
{
        //Smart Learning
        WPdb.CPSmFastAvgPoints1[0] = "";
        WPdb.CPSmFastAvgPoints1[1] = "";
        WPdb.CPSmSlowAvgPoints1[0] = "";
        WPdb.CPSmSlowAvgPoints1[1] = "";
        WPdb.CPSmSlowItpPoints1[0] = "";
        WPdb.CPSmSlowItpPoints1[1] = "";
        WPdb.CPSmSlowItpPoints1[2] = "";
        WPdb.CPSmSlowItpPoints1[3] = "";
        WPdb.CPSmSlowItpPoints1[4] = "";
        WPdb.CPSmSlowItpPoints1[5] = "";
        WPdb.CPSmSlowItpPoints1[6] = "";
        WPdb.CPSmSlowItpPoints1[7] = "";
        WPdb.CPSmSwitchTime1       = 0 ;
        
        WPdb.CPSmFastAvgPoints2[0] = "";
        WPdb.CPSmFastAvgPoints2[1] = "";
        WPdb.CPSmSlowAvgPoints2[0] = "";
        WPdb.CPSmSlowAvgPoints2[1] = "";
        WPdb.CPSmSlowItpPoints2[0] = "";
        WPdb.CPSmSlowItpPoints2[1] = "";
        WPdb.CPSmSlowItpPoints2[2] = "";
        WPdb.CPSmSlowItpPoints2[3] = "";
        WPdb.CPSmSlowItpPoints2[4] = "";
        WPdb.CPSmSlowItpPoints2[5] = "";
        WPdb.CPSmSlowItpPoints2[6] = "";
        WPdb.CPSmSlowItpPoints2[7] = "";
        WPdb.CPSmSwitchTime2       = 0 ;
          
        //Flash learning
        WPdb.CPFlFastPoints1[0] = "";
        WPdb.CPFlFastPoints1[1] = "";
        WPdb.CPFlDeviationPoints1[0] = "";
        WPdb.CPFlDeviationPoints1[1] = "";
        WPdb.CPFlDeviationPoints1[2] = "";
        WPdb.CPFlDeviationPoints1[3] = "";
        
        WPdb.CPFlFastPoints2[0] = "";
        WPdb.CPFlFastPoints2[1] = "";
        WPdb.CPFlDeviationPoints2[0] = "";
        WPdb.CPFlDeviationPoints2[1] = "";
        WPdb.CPFlDeviationPoints2[2] = "";
        WPdb.CPFlDeviationPoints2[3] = "";
...
int ScrnNumbNav=Scrn_No, Scrn_IO=Scrn_No;
  ...
  if (Scrn_No==SCREEN_RECIPE1 || Scrn_No==SCREEN_RECIPE2 || Scrn_No==SCREEN_RECIPE3 || Scrn_No==SCREEN_RECIPE4 || Scrn_No==SCREEN_IOCFG_IN1 || Scrn_No==SCREEN_IOCFG_IN2 || Scrn_No==SCREEN_IOCFG_IN3
      || Scrn_No==SCREEN_IOCFG_OUT1 || Scrn_No==SCREEN_IOCFG_OUT2 || Scrn_No==SCREEN_IOCFG_OUT3 || Scrn_No==SCREEN_IOCFG_OUT4 || Scrn_No==SCREEN_IOTEST_IN1 || Scrn_No==SCREEN_IOTEST_IN2 
      || Scrn_No==SCREEN_IOTEST_IN3 || Scrn_No==SCREEN_IOTEST_OUT1 || Scrn_No==SCREEN_IOTEST_OUT2 || Scrn_No==SCREEN_IOTEST_OUT3|| Scrn_No==SCREEN_IOTEST_OUT4 || Scrn_No==SCREEN_MP1 
...
// begindraw();
    ...
    switch (ScrnNumbNav)
    {
      case SCREEN_AUTOMATIC: 
              ...
              if ( tmpBTN6 != BTN6 || forced )
              {
                ...
                switch (Discharge_StateImage)
                {
                   ...
                   case DischPan:      widget_image (NAV_FCT_BUTTON_6, BTN_DISCH_PAN);       break;
      ...
      case SCREEN_RECIPES: 
              ...
              if (Scrn_No==SCREEN_IOCFG_OUT1 || Scrn_No==SCREEN_IOCFG_OUT2 || Scrn_No==SCREEN_IOCFG_OUT3 || Scrn_No==SCREEN_IOCFG_OUT4 
                || Scrn_No==SCREEN_IOTEST_OUT1 || Scrn_No==SCREEN_IOTEST_OUT2 || Scrn_No==SCREEN_IOTEST_OUT3 || Scrn_No==SCREEN_IOTEST_OUT4) 
...
//                  case SAVE_PAGES:      widget_image (NAV_FCT_BUTTON_9, BTN_SAVE);     break;
                  ...
                  case SCREEN_IOCFG_OUT4:   widget_image (NAV_FCT_BUTTON_9, BTN_PREV);     break;
                  ...
                  case SCREEN_IOTEST_OUT4:  widget_image (NAV_FCT_BUTTON_9, BTN_PREV);     break;
              ...
              {  
                ...
                {   
                  ...
                  case SCREEN_IOCFG_OUT3:   widget_image (NAV_FCT_BUTTON_10, BTN_NEXT);     break;
                  ...
                  case SCREEN_IOTEST_OUT3:  widget_image (NAV_FCT_BUTTON_10, BTN_NEXT);     break;
...
int Priority_Loop (int Priority,int ScaleNo)
{
...
/***Check if current Scale is in Fault***/
bool CheckFault(int ScaleNo)
{
 int mask[2],FaultOut[2][4],i;
  
  mask[Scale1] = 0x000003FF;
  mask[Scale2] = 0xFFFFFC00;
  
  for(i=0;i<=3;i++)
  {
   FaultOut[ScaleNo-1][i] = Fault_Type[i] & mask[ScaleNo-1];
   if(FaultOut[ScaleNo-1][i] > 0)
    {
     return(ON); //ON if fault present for the current scale
    }
  }   
 return(OFF); 
}
...
--  Side Effects      : 
--------------------------------------------------------------------------------*/
bool MessageDisplayOutput(string Msg, int Fault_No,int ScaleNo)
{
string backcolor[12], TempColor[12], Flt_Msg_Str[FltStringLen],TextAdd[255];
int i, Temp_Loop, High_Msg, Flt_Msg_Int,ScaleCheck,
  ...
  if ((Fault_No!=Step_Message)) 
  {
     ...
     High_Msg = Priority_Loop(PRIORITY,ScaleNo);
  ...
  if (High_Msg != -1)
  {
      if (PRIORITY == FLT_GEN)
      {
        if (OldMsg!=Msg || AutoPage_FS)
        {
          ...
          MessageColor[ScaleNo-1] = "red";
          HiResColor = "red";
          MessageText[ScaleNo-1] = concat ( trim(Fault_Txt[High_Msg]),space(1),TextAdd);
      ...
      if (PRIORITY == FLT_BLK && Fault_Type[FLT_GEN]==0)
      {
        if (OldMsg!=Msg || AutoPage_FS)
        {
          MessageColor[ScaleNo-1] = "red";
          HiResColor = "red";
          MessageText[ScaleNo-1] = concat ( trim(Fault_Txt[High_Msg])); 
      ...
      if(!CheckFault(ScaleNo))
      {
        MessageColor[ScaleNo-1] = CL_PT_GRAY;
        HiResColor = CL_PT_GRAY;
        MessageText[ScaleNo-1] = concat ( trim(StepMsg[ScaleNo-1]));
        ...
        if(ScaleNo == 1)
        {
         if(MessageTextTemp[Scale1] != MessageText[Scale1])
         {
          widget_background     (MESSAGE_DISPLAY1, MessageColor[ScaleNo-1]);
          widget_text           (MESSAGE_DISPLAY1, MessageText[ScaleNo-1]);
        ...
        if(ScaleNo == 2)
        {
         if(MessageTextTemp[Scale2] != MessageText[Scale2])
         {
          widget_background     (MESSAGE_DISPLAY2, MessageColor[ScaleNo-1]);
          widget_text           (MESSAGE_DISPLAY2, MessageText[ScaleNo-1]);
          widget_background     (HIRESWEIGHT, MessageColor[ScaleNo-1]);
 ...
 enddraw();
  ...
  StepMsg_MDisplay  = trim(StepMsg[ScaleNo-1]);
...
void MessageDisplay(string Msg, int Fault_No,int ScaleNo)
{
 if(Fault_No == 128)
  {
   MessageOutStp[ScaleNo-1] = trim (Msg);
   Fault_NoOutStp[ScaleNo-1] = trim(Fault_No);
  }
 else
  {
   MessageOutFault[ScaleNo-1] = trim (Msg);
   Fault_NoOutFault[ScaleNo-1] = trim(Fault_No);
   DisplayFault[ScaleNo-1] = ON;
  } 
 Message_DisplayFlag[ScaleNo-1]   = Execute;
}
...
--  Side Effects      : 
--------------------------------------------------------------------------------*/  
void Msg_Display_Refresh (int ScaleNo)
{
begindraw();
...
string Flt_Letter[1], Flt_No[3],FaultString[255];
 ...
 BIT      = Fault_No % 32; //Modulo
    ...
    MessageDisplay(FaultString,Fault_No,ScaleNo);
...
if (!FaultResetFlag)
{
 ...
 FaultLight_Set(); 
 Output_OFF(O_Fault);
 Msg_Display_Refresh(1);
 Msg_Display_Refresh(2);
 IOError = OFF;
}
...
/*   Create Input field                            */                                                            
/* *********************************************** */
void CreateDualInputField (int id, int font_size, int font_face, int format, int len, int decs, int align_center, string text, int width, int height, bool widget_lock )
{
  createwidget(id, WIDGET_TYPE_TEXT_OUTPUT);
    widget_style          (id,  TEXTINPUT_STYLE); 
    widget_size_policy    (id,  SIZE_POLICY_MINIMUM, SIZE_POLICY_FIXED);
    widget_fontsize       (id,  font_size);
    widget_fontface       (id,  font_face);
    widget_format         (id,  format, len, decs);
    widget_label          (id,  text); 
  if (align_center)  widget_text_alignment (id,  WIDGET_TEXT_ALIGN_CENTER);
  else               widget_text_alignment (id,  WIDGET_TEXT_ALIGN_VCENTER_R);
    widget_dimensions     (id,  width, height);
  if (widget_lock) widget_maximum_size     (id,  width, height);
    widget_content_margins (id, 0,0,0,0); 
...
void CreateOnOffBoxField (int id, int font_face, int alignment, bool image, string image_ON, string image_OFF)
{
  createwidget (id, WIDGET_TYPE_ONOFF);
    ...
    widget_content_margins (id, 0,0,0,0); 
      ...
      widget_image_index (id, image_ON, image);
      widget_image_index (id, image_OFF, !image);    
}
...
Recipe_Grayout[4]= (BulkOnTime ||(WP.FeedAlgoType != FeedStd && WP.FeedAlgoType != FeediQSim));  

Recipe_Grayout[5] = WP.FeedAlgoType != FeedStd || WP.BulkMode == ByTime ;

Recipe_Grayout[6]= !BulkOnTime;
...
//WP screen #2
Recipe_Grayout[13]= !(AOUTActive[AIO1] && AOUTActive[AIO2]);
Recipe_Grayout[14]= !(AOUTActive[AIO1] && AOUTActive[AIO2]);
Recipe_Grayout[15]= ON; 
Recipe_Grayout[16]= ON;
Recipe_Grayout[17] = ((!IO_Enable[O_StreamRegulator1]&&!IO_Enable[O_StreamRegulator2]) || MC.FeederType!= FeederB);
Recipe_Grayout[18]= (!IO_Enable[O_ExtendActuator1] || !IO_Enable[O_RetractActuator1]) && (!IO_Enable[O_ExtendActuator2] || !IO_Enable[O_RetractActuator2]);
   
Recipe_Grayout[19]= (SeqModeManager != stp_OFF_MODE);
Recipe_Grayout[21]=!IO_Enable[O_VibrDischDoor1] && !IO_Enable[O_VibrDischDoor2];
Recipe_Grayout[23] = !(WP.FeedAlgoType == FeedStd) && !(WP.FeedAlgoType == FeediQSim);
Recipe_Grayout[24]= (SeqModeManager != stp_OFF_MODE);  
...
Recipe_Grayout[36]= (!(IO_Enable[O_CatchGate1])&&!(IO_Enable[O_CatchGate2]));

//WP screen #4
Recipe_Grayout[37] = flexIOoff;
Recipe_Grayout[38] = flexIOoff;
Recipe_Grayout[39] = !IO_Enable[O_FlexIO_3];
Recipe_Grayout[40] = !IO_Enable[O_DischDoor12] && !IO_Enable[O_DischDoor22];
...
MP_Grayout[6]= 0;                                                                                                
MP_Grayout[7]= (!IO_Enable[I_DischDoorClose1] && !IO_Enable[I_DischDoorClose2]);
MP_Grayout[9]= (!IO_Enable[I_CatchGateOpen1] && !IO_Enable[I_CatchGateOpen2]);
...
MP_Grayout[33]= (!(MC.FeederType == FeederBSF));
...
//MC screen #2
MC_Grayout[13]= (UserSupOrLess); /* Production report port */ 
MC_Grayout[14]= (UserSupOrLess); /* Unit record print config */   
MC_Grayout[15]= (UserSupOrLess); /* Print port continuous output */ 
MC_Grayout[16]= UserSupOrLess || MC.PortETHCfg != ETHFctReportToPrinter;

if(MC_Grayout_16_Mem != MC_Grayout[16])
{
 MC_Grayout_16_Mem = MC_Grayout[16];
...
MC_Grayout[19]=(!IO_Enable[I_DischDoorClose1]&&!IO_Enable[I_DischDoorClose2]) || UserSupOrLess;
MC_Grayout[20] = UserSupOrLess;
if(WP.FeedAlgoType == FeedStd || WP.FeedAlgoType == FeediQSim) {MC_Grayout[21]= 0;}
else                                                           {MC_Grayout[21]= 1;}

//MC screen #3
MC_Grayout[25] = UserSupOrLess;
MC_Grayout[26] = UserSupOrLess;
MC_Grayout[27] = UserSupOrLess;
MC_Grayout[28] = UserSupOrLess;
MC_Grayout[29] = UserSupOrLess;
MC_Grayout[30] = UserSupOrLess || (!(IO_Enable[O_ExtendActuator1] && IO_Enable[O_RetractActuator1]) && !(IO_Enable[O_ExtendActuator2] && IO_Enable[O_RetractActuator2]));
MC_Grayout[33] = UserSupOrLess;
MC_Grayout[34] = UserSupOrLess;
MC_Grayout[35] = UserSupOrLess;
MC_Grayout[36] = UserSupOrLess || (!(IO_Enable[O_ExtendActuator1] && IO_Enable[O_RetractActuator1]) && !(IO_Enable[O_ExtendActuator2] && IO_Enable[O_RetractActuator2]));
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
   ...
   WP.Overfill1            = WPdb.Overfill1;
   WP.Overfill2            = WPdb.Overfill2;          
   WP.Cutoff1              = WPdb.Cutoff1;
   WP.Cutoff2              = WPdb.Cutoff2;
   WP.BulkCutoff1          = WPdb.BulkCutoff1;
   WP.BulkCutoff2          = WPdb.BulkCutoff2;
   ...
   WP.MinFeeding1         = WPdb.MinFeeding1;
   WP.MinFeeding2         = WPdb.MinFeeding2; 
   WP.MaxFeeding1         = WPdb.MaxFeeding1;
   WP.MaxFeeding2         = WPdb.MaxFeeding2; 
   WP.StreamRegConfig     = WPdb.StreamRegConfig; 
   WP.ActuatorPos1        = WPdb.ActuatorPos1;
   WP.ActuatorPos2        = WPdb.ActuatorPos2;
   ...
   if(WPdb.ProdTotalUnitsCnt[Scale1] >= (WPdb.ProdUnderWeightCnt[Scale1]  + WPdb.ProdOverWeightCnt[Scale1]))
    {
     ...
     WP.ProdTotalWeight_1     = WPdb.ProdTotalWeight_1; 
    }
   else
    {
   ...
   if(WPdb.ProdTotalUnitsCnt[Scale2] >= (WPdb.ProdUnderWeightCnt[Scale2]  + WPdb.ProdOverWeightCnt[Scale2]))
    {
     ...
     WP.ProdTotalWeight_2     = WPdb.ProdTotalWeight_2; 
    }
   else
    {
   ...
   WP.BFTime1               = WPdb.BFTime1;
   WP.BFTime2               = WPdb.BFTime2;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
   ...
   WPdb.Overfill1           = WP.Overfill1;  
   WPdb.Overfill2           = WP.Overfill2;         
   WPdb.Cutoff1             = WP.Cutoff1;
   WPdb.Cutoff2             = WP.Cutoff2;
   WPdb.BulkCutoff1          = WP.BulkCutoff1;
   WPdb.BulkCutoff2          = WP.BulkCutoff2;
   ...
   WPdb.MinFeeding1          = WP.MinFeeding1; 
   WPdb.MinFeeding2          = WP.MinFeeding2; 
   WPdb.MaxFeeding1          = WP.MaxFeeding1;
   WPdb.MaxFeeding2          = WP.MaxFeeding2; 
   WPdb.StreamRegConfig     = WP.StreamRegConfig; 
   WPdb.ActuatorPos1         = WP.ActuatorPos1;
   WPdb.ActuatorPos2         = WP.ActuatorPos2;
   ...
   WPdb.ProdTotalWeight_1           = WP.ProdTotalWeight_1;
   WPdb.ProdTotalWeight_2           = WP.ProdTotalWeight_2;
   ...
   WPdb.BFTime1                 = WP.BFTime1;
   WPdb.BFTime2                 = WP.BFTime2;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MC.ContPort            = MCdb.ContPort;
      MC.ContRate            = MCdb.ContRate;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MCdb.ContPort            = MC.ContPort;
      MCdb.ContRate            = MC.ContRate;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MP.BSFBulkOffDelay  = MPdb.BSFBulkOffDelay;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MPdb.BSFBulkOffDelay  = MP.BSFBulkOffDelay;
...
IO_Name[2]  = lsub(TX_I_PRESSURESW);
IO_Name[3]  = lsub(TX_I_MOTORFAILURE);
IO_Name[4]  = concat(lsub(TX_I_LOWLEVELSENSOR)," 1");
IO_Name[5]  = concat(lsub(TX_I_DISCHDOORCLOSE)," 1");
IO_Name[6]  = concat(lsub(TX_I_CATCHGATEOPEN)," 1");

IO_Name[7]  = lsub(TX_I_DISCHREQ);
IO_Name[8]  = lsub(TX_I_HOPPEREMPTY);
IO_Name[9]  = lsub(TX_I_HIGHLEVELSENSOR);
IO_Name[10] = concat(lsub(TX_I_LOWLEVELSENSOR)," 2");
IO_Name[11] = concat(lsub(TX_I_DISCHDOORCLOSE)," 2");
IO_Name[12] = concat(lsub(TX_I_CATCHGATEOPEN)," 2");

IO_Name[13] = lsub(TX_I_START);
IO_Name[14] = lsub(TX_I_STOP);
IO_Name[15] = concat(lsub(TX_I_MANDISCH)," 1");
IO_Name[16] = lsub(TX_I_EMPTYBAGREADY);
IO_Name[17] = lsub(TX_I_CLEARFAULTS);
IO_Name[18] = lsub(TX_I_HOLDCYCLE);

IO_Name[19] = lsub(TX_I_BAGATKICKER);
IO_Name[20] = lsub(TX_I_KICKERRETRACTED);
IO_Name[21] = concat(lsub(TX_I_MANDISCH)," 2");
IO_Name[22] = lsub(TX_I_RESETOUTOFTOLALARM);
IO_Name[23] = lsub(TX_I_BATCHEND);
...
//Outputs
IO_Name[37] = concat(lsub(TX_O_BULKFEED)," 1");
IO_Name[38] = concat(lsub(TX_O_DRIBBLEFEED)," 1");
IO_Name[39] = concat(lsub(TX_O_CATCHGATE)," 1");
IO_Name[40] = concat(lsub(TX_O_STREAMREGULATOR)," 1");
IO_Name[41] = concat(lsub(TX_O_DISCHDOOR)," 1");
IO_Name[42] = concat(lsub(TX_O_VIBRDISCHDOOR)," 1");

IO_Name[43] = concat(lsub(TX_O_BULKFEED)," 2");
IO_Name[44] = concat(lsub(TX_O_DRIBBLEFEED)," 2");
IO_Name[45] = concat(lsub(TX_O_CATCHGATE)," 2");
IO_Name[46] = concat(lsub(TX_O_STREAMREGULATOR)," 2");
IO_Name[47] = concat(lsub(TX_O_DISCHDOOR)," 2");
IO_Name[48] = concat(lsub(TX_O_VIBRDISCHDOOR)," 2");

IO_Name[49] = lsub(TX_O_DISCHREADY);
IO_Name[50] = lsub(TX_O_HOPFILLREQ);
IO_Name[51] = lsub(TX_O_KNOCKHAMMER);
IO_Name[52] = lsub(TX_O_ACTIVATEKICKER);
IO_Name[53] = concat(lsub(TX_O_DISCHDOOR), " #1-2");
//IO_Name[53] = lsub(TX_O_CLOSINGCNV);
IO_Name[54] = concat(lsub(TX_CLEANOUTCOMPLETED)," 1");

IO_Name[55] = lsub(TX_O_DISCHARGE);
IO_Name[56] = lsub(TX_O_BAGHOLDER);
IO_Name[57] = lsub(TX_O_INFLATEBAG);
IO_Name[58] = lsub(TX_O_BAGRELEASE);
IO_Name[59] = concat(lsub(TX_O_DISCHDOOR), " #2-2");
//IO_Name[59] = lsub(TX_O_BATCHENDED);
IO_Name[60] = concat(lsub(TX_CLEANOUTCOMPLETED)," 2");

IO_Name[61] = lsub(TX_O_FAULT);
IO_Name[62] = concat(lsub(TX_O_LOWLEVEL)," 1");
IO_Name[63] = concat(lsub(TX_O_OUTOFTOLERANCE)," 1");
IO_Name[64] = concat(lsub(TX_O_OVERWEIGHTALARM)," 1");
IO_Name[65] = concat(lsub(TX_O_UNDERWEIGHTALARM)," 1");
IO_Name[66] = lsub(TX_O_EXTERNALBAGCOUNTER);

IO_Name[67] = lsub(TX_O_CYCLEINPROGRESS);
IO_Name[68] = concat(lsub(TX_O_LOWLEVEL)," 2");
IO_Name[69] = concat(lsub(TX_O_OUTOFTOLERANCE)," 2");
IO_Name[70] = concat(lsub(TX_O_OVERWEIGHTALARM)," 2");
IO_Name[71] = concat(lsub(TX_O_UNDERWEIGHTALARM)," 2");
IO_Name[72] = lsub(TX_O_CYCLEINHOLD);

IO_Name[73] = concat(lsub(TX_O_EXTENDACTUATORS)," 1");
IO_Name[74] = concat(lsub(TX_O_RETRACTACTUATORS)," 1");
IO_Name[75] = lsub(TX_O_WPNUMBOUT1);
IO_Name[76] = lsub(TX_O_WPNUMBOUT2);
IO_Name[77] = lsub(TX_O_WPNUMBOUT3);
IO_Name[78] = lsub(TX_O_WPNUMBOUT4);

IO_Name[79] = concat(lsub(TX_O_EXTENDACTUATORS)," 2");
IO_Name[80] = concat(lsub(TX_O_RETRACTACTUATORS)," 2");
IO_Name[81] = lsub(TX_O_FLEXIO_1);
IO_Name[82] = lsub(TX_O_FLEXIO_2);
IO_Name[83] = lsub(TX_O_FLEXIO_3);
IO_Name[84] = lsub(TX_O_BATCHENDED);
...
Fault_Txt[FltG_CatchGateNotOpen]  = concat("#1 ",lsub(TX_FltG_CatchGateNotOpen));
Fault_Txt[FltG_CatchGateNotOpen2] = concat("#2 ",lsub(TX_FltG_CatchGateNotOpen));
Fault_Txt[FltG_SystemError] = lsub(TX_FltG_SystemError);
Fault_Txt[FltG_DischDoorNotClosed] = concat("#1 ",lsub(TX_FltG_DischDoorNotClosed));
Fault_Txt[FltG_DischDoorNotClosed2] = concat("#2 ",lsub(TX_FltG_DischDoorNotClosed));
Fault_Txt[FltG_ExceededCapacity] = concat("#1 ",lsub(TX_FltG_ExceededCapacity));
Fault_Txt[FltG_ExceededCapacity2] = concat("#2 ",lsub(TX_FltG_ExceededCapacity));
Fault_Txt[FltG_MotorFailure] = lsub(TX_FltG_MotorFailure);

//Blocking Faults
Fault_Txt[FltB_UnderWeight] = concat("#1 ",lsub(TX_FltB_UnderWeight));
Fault_Txt[FltB_OverWeight] = concat("#1 ",lsub(TX_FltB_OverWeight));
Fault_Txt[FltB_ExceededCapacity] = concat("#1 ",lsub(TX_FltG_ExceededCapacity));
Fault_Txt[FltB_UnderWeight2] = concat("#2 ",lsub(TX_FltB_UnderWeight));
Fault_Txt[FltB_OverWeight2] = concat("#2 ",lsub(TX_FltB_OverWeight));
Fault_Txt[FltB_ExceededCapacity2] = concat("#2 ",lsub(TX_FltG_ExceededCapacity));

//Stopping Faults
Fault_Txt[FltS_StabTooLong] = concat("#1 ",lsub(TX_FltS_StabTooLong));
Fault_Txt[FltS_StabTooLong2] = concat("#2 ",lsub(TX_FltS_StabTooLong));
Fault_Txt[FltS_ZeroOutOfRange] = concat("#1 ",lsub(TX_FltS_ZeroOutOfRange));
Fault_Txt[FltS_ZeroOutOfRange2] = concat("#2 ",lsub(TX_FltS_ZeroOutOfRange));
...
Fault_Txt[FltW_SlowCycle] = concat("#1 ",lsub(TX_FltW_SlowCycle));
Fault_Txt[FltW_SlowCycle2] = concat("#2 ",lsub(TX_FltW_SlowCycle));
Fault_Txt[FltW_BulkBlankTooLong] = concat("#1 ",lsub(TX_FltW_BulkBlankTooLong));
Fault_Txt[FltW_BulkBlankTooLong2] = concat("#2 ",lsub(TX_FltW_BulkBlankTooLong));
Fault_Txt[FltW_DribbleBlankTooLong] = concat("#1 ",lsub(TX_FltW_DribbleBlankTooLong));
Fault_Txt[FltW_DribbleBlankTooLong2] = concat("#2 ",lsub(TX_FltW_DribbleBlankTooLong));
Fault_Txt[FltW_SystemWarning] = lsub(TX_FltW_SystemWarning);
Fault_Txt[FltW_HopperLowLevel] = concat("#1 ",lsub(TX_FltS_HopperLowLevel));
Fault_Txt[FltW_HopperLowLevel2] = concat("#2 ",lsub(TX_FltS_HopperLowLevel));
...
/*       Auto page                  */
/* ******************************** */
int Scrn_No_Temp,FirstNameNo,i;
  ...
  begindraw();
   ...
   if (Scrn_No == SCREEN_IOCFG_OUT4 || Scrn_No == SCREEN_IOTEST_OUT4) Scrn_No_Temp = SCREEN_OUTPUT_PAGE4;
   ...
   switch(Scrn_No_Temp)
   {
    ...
    case SCREEN_RECIPE1:  
      ...
      //  Header
        ...
        // First Column                                
              ...
              // 2nd Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW2_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPOVERFILL);
              widget_text       (RECIPE_ROW2_ID, lsub(TX_WPOVERFILL));
              widget_label      (RECIPE_ROW2_INPUT, lsub(TX_WPOVERFILL));                                              
              // Third Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW3_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPCUTOFFWEIGHT);
              widget_text       (RECIPE_ROW3_ID, lsub(TX_WPCUTOFFWEIGHT));
              widget_label      (RECIPE_ROW3_INPUT, lsub(TX_WPCUTOFFWEIGHT));                                              
              // Fourth Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW4_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPDRIBBLEFEEDWEIGHT);
              ...
              // Sixth Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW6_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPBULKFEEDTIME);
        ...
        // Second Column                               
              ...
              // Third Row Info 
              //widget_text       (RECIPE_ROW9_ID, "");
              //widget_label      (RECIPE_ROW9_INPUT, "");                                              
              // Fourth Row Info 
    ...
    case SCREEN_RECIPE2:  
      ...
      //  Header
        ...
        // First Column                                
              ...
              // First Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW13_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPMINFEEDING);
              widget_text       (RECIPE_ROW13_ID, lsub(TX_WPMINFEEDING));
              widget_label      (RECIPE_ROW13_INPUT, lsub(TX_WPMINFEEDING));        
              // 2nd Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW14_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPMAXFEEDING);
              widget_text       (RECIPE_ROW14_ID, lsub(TX_WPMAXFEEDING));
              widget_label      (RECIPE_ROW14_INPUT, lsub(TX_WPMAXFEEDING));                                               
              // Third Row Info 
              //widget_text       (RECIPE_ROW15_ID, "");
              //widget_label      (RECIPE_ROW15_INPUT,"");                                              
              // Fourth Row Info 
              //widget_text       (RECIPE_ROW16_ID,"");
              //widget_label      (RECIPE_ROW16_INPUT,"");                                               
              // Fifth Row Info 
              ...
              // Sixth Row Info 
              RecipeRowString[RecipeDualInIndex[RECIPE_ROW18_INPUT - RECIPE_INPUT_FIRST]] = lsub(TX_WPACTUATORPOS);
        ...
        // Second Column                               
              ...
              widget_text       (RECIPE_ROW24_SELECT, "#1", Scale1Only);
              widget_text       (RECIPE_ROW24_SELECT, "#2", Scale2Only);
              widget_text       (RECIPE_ROW24_SELECT, "#1 & #2", Scale1And2);
     break;
    ...
    case SCREEN_INPUT_PAGE1:
              ...
              for(i=0;i<12;i++)
              { 
               widget_text       (IO_ID_ROW1+i,     IO_Name[FirstNameNo+i]);
               widget_label      (IO_INPUT_ROW1+i,  IO_Name[FirstNameNo+i]);
              } 
    ...
    case SCREEN_OUTPUT_PAGE4:
              ...
              switch (Scrn_No)
              {
                case SCREEN_IOCFG_OUT4: widget_text(IO_CHECK_HEADER1, lsub(TX_IO_ENABLE));
                                        widget_text(IO_CHECK_HEADER2, lsub(TX_IO_ENABLE));
                                     break;
    ...
    case SCREEN_MP3:  
      ...
      /* ******************************** */ 
        ...
        // Second Column   
              ...
              // 2nd Row Info
              widget_text       (MP_ROW33_ID,    lsub(TX_BSFBULKDELAY));
              widget_label      (MP_ROW33_INPUT, lsub(TX_BSFBULKDELAY));                             
    ...
    case SCREEN_MC1:  
      ...
      /* ******************************** */ 
        ...
        // First Column                                
              ...
              // Fifth Row Info 
              widget_text       (MC_ROW5_ID,    lsub(TX_MCDISCHREQDEBOUNCING));
              widget_label      (MC_ROW5_INPUT, lsub(TX_MCDISCHREQDEBOUNCING));
        ...
        // Second Column                               
              ...
              // 2nd Row Info 
              widget_text       (MC_ROW8_ID,    lsub(TX_MCNUMBEROFWP));
              widget_label      (MC_ROW8_INPUT, lsub(TX_MCNUMBEROFWP));
                                               
              // Third Row Info 
              widget_text       (MC_ROW9_ID,     lsub(TX_MCCFGBAGHOLDER));
              widget_text       (MC_ROW9_SELECT, lsub(TX_WPNO), 0);
              widget_text       (MC_ROW9_SELECT, lsub(TX_MCHolderS4), 1);
              widget_text       (MC_ROW9_SELECT, lsub(TX_MCHolderBirdBeak), 2);
                                            
               // Fourth Row Info
               widget_text       (MC_ROW10_ID,    lsub(TX_MCTRIALDAYS));
               widget_label      (MC_ROW10_INPUT, lsub(TX_MCTRIALDAYS));
    ...
    case SCREEN_MC2:  
      ...
      /* ******************************** */ 
        ...
        // First Column                                
              ...
              // Third Row Info 
              widget_text       (MC_ROW15_ID,     lsub(TX_PortEthernet));
              widget_text       (MC_ROW15_SELECT, lsub(TX_MCNone), ETHFctNone);
              widget_text       (MC_ROW15_SELECT, lsub(TX_FctReportToPrinter),ETHFctReportToPrinter); 
                                              
              // Fourth Row Info 
              widget_text       (MC_ROW16_ID,     lsub(TX_PrinterIP));
              widget_text       (MC_ROW16_INPUT, lsub(TX_PrinterIP));
                                                 
              // Fifth Row Info 
              widget_text       (MC_ROW17_ID,     lsub(TX_MCCONTOUTPORT));
              widget_text       (MC_ROW17_SELECT, lsub(TX_MCNone), CONTPortNone);
              widget_text       (MC_ROW17_SELECT, lsub(TX_ContOutSIM3_4),CONTPortSIM);
              widget_text       (MC_ROW17_SELECT, lsub(TX_PortEthernet), CONTPortETH);
   
                                              
              // Sixth Row Info 
              widget_text       (MC_ROW18_ID,     lsub(TX_MCCONTOUTRATE));
              widget_text       (MC_ROW18_INPUT,  lsub(TX_MCCONTOUTRATE));
        ...
        // Second Column                               
              ...
              // Third Row Info 
              widget_text       (MC_ROW21_ID,     lsub(TX_MCAUTOSAVECORRECTION));
              widget_text       (MC_ROW21_SELECT, lsub(TX_WPNO), 0);
              widget_text       (MC_ROW21_SELECT, lsub(TX_WPYES), 1);
                                            
              // Fourth Row Info 
              widget_text       (MC_ROW22_ID,    lsub(TX_MCPLANTLINENAME));
              widget_label      (MC_ROW22_INPUT, lsub(TX_MCPLANTLINENAME));
      ...
      /* ******************************** */ 
        ...
        // First Column                                
             ...
             // First Row Info 
              ...
              // Third Row Info 
              widget_text       (MC_ROW27_ID,    lsub(TX_MCSERIALNBR));
              widget_label      (MC_ROW27_INPUT, lsub(TX_MCSERIALNBR));
             ...
             // Fifth Row Info 
              ...
              // Sixth Row Info 
              widget_text       (MC_ROW30_ID,    lsub(TX_MCACTUATORPOSINVERT));
              widget_text       (MC_ROW30_SELECT, lsub(TX_WPNO), 0);
              widget_text       (MC_ROW30_SELECT, lsub(TX_WPYES), 1);
        ...
        // Second Column                               
              ...
              // Fifth Row Info 
              widget_text       (MC_ROW35_ID,    lsub(TX_MCUSBMOUSE)); 
              widget_label      (MC_ROW35_INPUT, lsub(TX_MCUSBMOUSE));
              
              // Sixth Row Info 
              widget_text       (MC_ROW36_ID,     lsub(TX_MCACTUATOREXTENDLIMIT));
              widget_label      (MC_ROW36_INPUT, lsub(TX_MCACTUATOREXTENDLIMIT));
    ...
    case SCREEN_FLEXIO_1:  
      ...
      LoadIONames();
        ...
        // First Column                                 
           ...
           widget_text       (FLEX_ROW1_SELECT, concat("W1-",lsub(TX_FLEXSETPOINT)),2);
           widget_text       (FLEX_ROW1_SELECT, concat("W2-",lsub(TX_FLEXSETPOINT)),3);
        ...
        // Second Column  
           ...
           widget_text       (FLEX_ROW7_SELECT,  concat("W1-",lsub(TX_FLEXSETPOINT)),2);
           widget_text       (FLEX_ROW7_SELECT,  concat("W2-",lsub(TX_FLEXSETPOINT)),3);
    ...
    case SCREEN_FLEXIO_2:  
      ...
      /* ******************************** */ 
        ...
        // First Column                                 
           ...
           widget_text       (FLEX_ROW13_SELECT, concat("W1-",lsub(TX_FLEXSETPOINT)),2);
           widget_text       (FLEX_ROW13_SELECT, concat("W2-",lsub(TX_FLEXSETPOINT)),3);
        ...
        // Second Column  
           ...
           widget_text       (FLEX_ROW19_SELECT, concat("W1-",lsub(TX_FLEXSETPOINT)),2);
           widget_text       (FLEX_ROW19_SELECT, concat("W2-",lsub(TX_FLEXSETPOINT)),3);
      ...
      // Header
        ...
        // Second Column                               
            ...
            if(ReportOnCumulated)
               ...
               {
                // 2nd Row Info 
                widget_text       (REPORT_ROW9_ID, lsub(TX_ID_UNITS_DONE));
                // Third Row Info 
                widget_text       (REPORT_ROW10_ID, lsub(TX_ID_UNITS_ACCEPTED));
                // Fourth Row Info 
                widget_text       (REPORT_ROW11_ID, lsub(TX_ID_UNDERWEIGHT));
                // Fifth Row Info 
                widget_text       (REPORT_ROW12_ID, lsub(TX_ID_OVERWEIGHT));
                // Sixth Row Info 
                widget_text       (REPORT_ROW13_ID, lsub(TX_ID_TOTAL_WEIGHT)); 
               }
    ...
    case SCREEN_INFO:
      ...
      // First Column
           ...
           // MAC address
           lcaseAddress = lsub(TX_IO_ADDRESS);
           lcaseAddress2 = lcase("AbCd");
...
--  Side Effects      : 
--------------------------------------------------------------------------*/
void Create1or2PopupBox (void)         
{
  begindraw();
    createwindow (W1OR2_POPUP_WINDOW, WINDOW_TYPE_POPUP, "Popup Window");//  windowstyle (WD_INFO_BOX, ST_BOX_STYLE);
    windowstyle (W1OR2_POPUP_WINDOW, POPUP_OUTBOX_STYLE);
      
      beginlayout (LAYOUT_TYPE_VBOX,0);
      
        beginlayout (LAYOUT_TYPE_VBOX,0);
         //Info Text
          createwidget (W1OR2_POPUP, WIDGET_TYPE_TEXT_OUTPUT);
            widget_size_policy    (W1OR2_POPUP,  SIZE_POLICY_MINIMUM, SIZE_POLICY_MINIMUM);
            widget_style          (W1OR2_POPUP,  POPUP_BOXTEXT_STYLE);
            widget_fontsize       (W1OR2_POPUP,  30);                                
            widget_text_alignment (W1OR2_POPUP,  WIDGET_TEXT_ALIGN_HCENTER);
        endlayout();
       
        //Button Weigher #1 
        beginlayout(LAYOUT_TYPE_HBOX, STD_EDGE_DISTANCE_NAVIG, FCT_BUTTON_DISTANCE_NAVIG);
          createwidget (ID_STRETCH, WIDGET_TYPE_STRETCH); 
          createwidget (W1_BOX,     WIDGET_TYPE_TEXT_OUTPUT);
          ...
          //Button Weigher #2        
          createwidget (ID_STRETCH, WIDGET_TYPE_STRETCH); 
          createwidget (W2_BOX, WIDGET_TYPE_TEXT_OUTPUT);
...
void PreSave_Data (int WP_No,bool reset)
{
  ...
  WP.ProdTotalWeight_1     = ProdTotalWeight1;
  WP.ProdTotalWeight_2     = ProdTotalWeight2;
  SaveWP2Db();
  if(reset)
   {
    ResetData(OFF);
...
{  
  ...
  ProdTotalWeight1            = WP.ProdTotalWeight_1;
  ProdTotalWeight2            = WP.ProdTotalWeight_2;
...
CheckOK = ON;
  ...
  { 
   ...
   //if (ScaleDriver_No[1] != iQ_Feeding) {SystemErrorCode = "412 Scale #2 only iQFeeding driver permitted"; CheckOK = OFF;}
  }

  else
  {
    if (ScaleDriver_No[0] != iQ_Sim) {SystemErrorCode = "421 Scale #1 iQSim driver not installed"; CheckOK = OFF;}
    //if (ScaleDriver_No[1] != iQ_Sim) {SystemErrorCode = "422 Scale #2 iQSim driver not installed"; CheckOK = OFF;}
  }
  
  if (ScaleDriver_No[2] != Lbs && ScaleDriver_No[2] != No_Scale){SystemErrorCode = "413 Scale #3 only LBS driver permitted"; CheckOK = OFF;}
...
/******Switch Scale either for 1-2 or kg-lb******/
/****** 1= Scale 1 in kg                   ******/
/****** 2= Scale 2 in kg                   ******/
/****** 3= Scale 1 in lb                   ******/
/****** 4= Scale 2 in lb                   ******/
int SwitchScale(bool tgltype)
{
int ActualScale, DesiredScale;

 
 ActualScale = indicatedscale();
 DesiredScale = ActualScale;
  
  if(tgltype) //tgltype ON = kg-lb, OFF = 1-2
  {
   if((WP.Unit == "kg" && (ActualScale == 3 || ActualScale == 4)) || (WP.Unit == "lb" && (ActualScale == 1 || ActualScale == 2)))
   {
    switch(ActualScale)
    {
     case 1: DesiredScale = 3; break;
     case 2: DesiredScale = 4; break;
     case 3: DesiredScale = 1; break;
     case 4: DesiredScale = 2; break;
    }
   } 
  }  
  ...
  else
  {
   switch(ActualScale)
   {
    case 1: DesiredScale = 2; break;
    case 2: DesiredScale = 1; break;
    case 3: DesiredScale = 4; break;
    case 4: DesiredScale = 3; break;
   }
  
  }
  
  #if defined __SIMULATION__
   if(DesiredScale >= 3) DesiredScale = 1;
  #endif
  
  if(DesiredScale != ActualScale) indicatescale(DesiredScale);
  ScaleSwitched = ON;
  return(DesiredScale);
}
