 ...
 int     WPTransferSiloTemp,WPBulkModeIPTemp,MCWeighingModeTemp,WPPresetBatchTempMem,WPBatchTargetTempMem; 
  ...
  // First Column                                
        ...
        if(TargetWeightTemp[Scale1] != TargetWeight[Scale1] || !FirstPassDone || TargetWeightBatchTempTemp != TargetWeightBatchTemp)
        {
         ...
         if(BulkByWeightON)BulkByWeightON = OFF; //SADM2 2020 November
         }
         else
         {
        ...
        // Third Row Info 
        if(ProdTotalWeightTemp[Scale1] != ProdTotalWeight || !FirstPassDone || BatchTotalWeightTemp != Batch_TotalWeight)  //SADM2 2020 NOVEMBER
        {
         if(BulkByWeightON || BatchByDischON || BulkByDischON || BatchByWeightON)
         {
           if(UnitTemp == "t") 
           {
             ProdTotalWeightUnitTon = Batch_TotalWeight/1000;  //SADM2 2020 NOVEMBER
         ...
         else
         {
           if(UnitTemp == "t") 
           {
             ProdTotalWeightUnitTon = ProdTotalWeight/1000;  //SADM2 2020 NOVEMBER
             widget_text (AUTO_ROW3_OUTPUT, concat(adj(ProdTotalWeightUnitTon,6,dp+1),UnitTemp));
           }
           else 
           {
            widget_text (AUTO_ROW3_OUTPUT, concat(adj(ProdTotalWeight,6,dp),WP.Unit));  //SADM2 2020 NOVEMBER 
           }
         }
         ProdTotalWeightTemp[Scale1] = ProdTotalWeight; //SAMD2 2020 NOVEMBER 
