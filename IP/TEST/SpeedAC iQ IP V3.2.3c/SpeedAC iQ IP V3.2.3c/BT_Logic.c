...
BatchCancelled = OFF;
BatchEndDone = OFF;
TotalWeightBatchTemp = Batch_TotalWeight;  //SADM2 2020 NOVEMBER
}
...
if(WeightSampled[Scale1] && !LowLevel[Scale1] && !WeighmentAborted && SqareSumWeight[Scale1]<2000000000 && SeqBatchingMode != stp_LastTwoWeighment)
  ...
  StdDev[Scale1] = sqrt(ValueToSqrt);
    ...
    ResetData(ON);
       ...
       if(WeightSampled[Scale1])
       {
       ProdTotalWeight = ProdTotalWeight + (CheckedWeight[Scale1] - WeightDisch);  //SADM2020 NOVEMBER
       if((Batch.Preset > 0 || WP.TargetWeightIP > 0 || BulkByDischON) && (MC.WeighingMode == Bulk || MC.WeighingMode == Batching))
       {
         Batch_TotalWeight = Batch_TotalWeight + CheckedWeight[Scale1];  //SADM2 2020 NOVEMBER
         ProdRemainWeight = (WP.TargetWeightIP+DTFWeight)- Batch_TotalWeight;  //SADM2 2020 NOVEMBER
       }
       ...
       {
       ProdTotalWeight = ProdTotalWeight + (TargetWeight[Scale1] - WeightDisch);  //SADM2020 NOVEMBER
       if((Batch.Preset > 0 || WP.TargetWeightIP >  0 || BulkByDischON) && (MC.WeighingMode == Bulk || MC.WeighingMode == Batching))
       {
         Batch_TotalWeight = Batch_TotalWeight + TargetWeight[Scale1];  //SADM2 2020 NOVEMBER
         ProdRemainWeight = (WP.TargetWeightIP+DTFWeight)- Batch_TotalWeight; //SADM2 2020 NOVEMBER
       }
       ...
       //if(ProdTotalWeightLT[Scale1] >= 21474836.48 || ProdTotalWeightLT[Scale1] == -21474836.48)ProdTotalWeightLT[Scale1]= 0;
       ProdTotalWeightLT = ProdTotalWeightLT + (TargetWeight[Scale1]/FACTOR); //SADM2 2020 NOVEMBER
...
{   
  ...
  switch (SeqPulseTotalWeight)
  {
    case stp_InitPTW:
         ...
         if(TotalWeightPulseTemp != ProdTotalWeightLT)  //SADM2 2020 NOVEMBER
         {
           TotalWeightPulseTemp = ProdTotalWeightLT; //SADM2 2020 NOVEMBER
           AccPulseTotalWeight = ProdTotalWeightLT-LastPulseTotalWeight+PulseTotalWeightRemain; //SADM2 2020 NOVEMBER
         }
    ...
    case stp_StopPTW:
         ...
         {
           SeqPulseTotalWeightMEM = SeqPulseTotalWeight;
           LastPulseTotalWeight = ProdTotalWeightLT;  //SADM2 2020 NOVEMBER  
...
{ 
   ...
   {       
        case stp_InitWeighment:
             ...
             if(!BatchEndDone)
             {
               if(Batch_TotalWeight <= (WP.TargetWeightIP+DTFWeight))  //SADM2 2020 NOVEMBER
                {
                 ProdRemainWeightCheck = (WP.TargetWeightIP+DTFWeight)-Batch_TotalWeight; //SADM2 2020 NOVEMBER
                }
        ...
        case stp_RegularWeighment:
             ...
             if(TotalWeightBatchTemp != Batch_TotalWeight)  //SADM2 2020 NOVEMBER
             {
               TotalWeightBatchTemp = Batch_TotalWeight;  //SADM2 2020 NOVEMBER
        ...
        case stp_LastTwoWeighment:
             ...
             if(TotalWeightBatchTemp != Batch_TotalWeight)  //SADM2 2020 NOVEMBER
             {
               ...
               {
                 CutoffTemp = (WP.TargetWeightIP+DTFWeight-Batch_TotalWeight)-(WP.NominalWeight-WP.Cutoff); //SADM2 2020 NOVEMBER
                 TargetWeightBatchTemp = CutoffTemp+(WP.NominalWeight-WP.Cutoff); 
               }
   ...
   {
          OffModeOn = ON;
          if(!BatchEndDone)ProdRemainWeight = (WP.TargetWeightIP+DTFWeight)- Batch_TotalWeight;  //SADM2 2020 NOVEMBER
...
{
  ProdTotalWeight = 0; //SADM2 2020 NOVEMBER
