#ifndef __check_e_driver_h__
#define __check_e_driver_h__


//#warning "das Debug-Flag muss in der Engültigen Version auskommeniert werden!!"
//#define CHECK_E_DRIVER_DEBUG
...
J.Pauel      25.08.17          2. Added the pass parameters to the CheckAnyValidEvalParameter function.
 ...
 J.Pauel     28.02.19   1.5.8   1. Insert the zeroing filter in the evaluation parameters!
                                2. Inserted in function check_weight_msg LOGGING_EXTENTION_DEBUG. (to test)
             21.06.19   1.5.9   3. For the CheckAnyValidEvalParameter function, pass the handle and the scale number as the transfer parameter
                                4. With the call of the binary "check_e_driver.obj" still the version number in the file name with inserted! (check_e_driver_V1.5.8.obj)
                                5. Renamed the RebootTerminal function to CheckDriverRebootTerminal

 M.Neuendorf 19.03.20   1.5.10  1. Added interface to filter optimization
                                2. Added interface to query the voltage of the load cell (check_static_weight_ex)
                                3. Added interface to set values of the evaluation parameters (check_set_parameter_details)
*/

#define  CHECK_E_DRIVER_MAJOR         "1"
#define  CHECK_E_DRIVER_MINOR         "5"
#define  CHECK_E_DRIVER_PATCH         "10"
...
int           ZeroSettingFilterTime;       //  pa+ - 30.04.2019
...
enum
{
ID_CHECK_PARNAME_EVALUATION_PARAMETERS,
ID_CHECK_PARNAME_PRIMARY_FILTER_TIME,
ID_CHECK_PARNAME_EVALUATION_FILTER_SIZE,
ID_CHECK_PARNAME_ZERO_SETTING_FILTER_TIME,
ID_CHECK_PARNAME_BELT_SPEED,
ID_CHECK_PARNAME_CORRECTING_POLYNOMIAL,  // Include alpha / beta and weight offset
ID_CHECK_PARNAME_MEASUREMENT_START_OFFSET,
ID_CHECK_PARNAME_MEASUREMENT_STOP_OFFSET,
ID_CHECK_PARNAME_MEASUREMENT_MANUAL_THRESHOLD,
ID_CHECK_PARNAME_MEASURING_TIME,
ID_CHECK_PARNAME_MINIMUM_WEIGHT,
ID_CHECK_PARNAME_MAXIMUM_WEIGHT,
ID_CHECK_PARNAME_VALID,
};
...
#########################################################################
#  set pusher assigment                                                 #
#########################################################################
bool check_set_pusher_assignment (int     handle,
                                  int     scaleno,
                                  string  assignment)
...
#########################################################################
#  writes the evaluation parameter details of the given parameter set   #
# set_mask is a bit vector of the values to write from enum             #
# ID_CHECK_PARNAME_*                                                    #
#########################################################################
bool check_set_parameter_details
  (int      handle,
  int       scaleno,
  int       parameterno,
  DynEvaPar ep,
...
bool check_static_weight_ex (int    handle,
                          int   scaleno,
                          float *static_gross,
                          float *static_tare,
                          float *static_net,
...
#########################################################################
#  Function to reboot the Scale after leaving service mode               #
#########################################################################
void CheckDriverRebootTerminal (void)
...
#########################################################################
//bool CheckAnyValidEvalParameter ();    // pa+ - 21.06.2019
bool CheckAnyValidEvalParameter (int handle,
                                 int scaleno);

*/

/***********************************************************************/
// start recording of weights for filter optimization (analysis)
/***********************************************************************/
bool check_start_recording
     (int handle,
     int scaleno,
     int record_count);

/***********************************************************************/
// abort any previously started recording of weights
/***********************************************************************/
bool check_abort_recording
     (int handle,
     int scaleno);
...
/***********************************************************************/
// start analyzing the filter settings with automatic settings
/***********************************************************************/
bool check_start_analysis
     (int handle,
     int scaleno,
...
/***********************************************************************/
// start analyzing the filter settings with manual settings
// any arguments (except match_percentage) will be ignored if set to zero
/***********************************************************************/
bool check_start_analysis_ext
     (int handle,
     int scaleno,
     int primary_min_time,
     int primary_max_time,
     int eval_min_size,
     int eval_max_size,
     float belt_length,
     float belt_speed,
     float max_obj_length,
     float match_percentage,
     bool force_full_search);

/***********************************************************************/
// abort any running filter optimization
/***********************************************************************/
bool check_abort_analysis
     (int handle,
     int scaleno);

/***********************************************************************/
// fetch the progress of an ongoing filter optimization process
/***********************************************************************/
bool check_fetch_analysis_progress
     (int handle,
     int scaleno,
     float *percentage,
     bool *finished);
...
/***********************************************************************/
// fetch the result of a successfully finished filter optimization
/***********************************************************************/
bool check_fetch_analysis_result
     (int handle,
     int scaleno,
     int *primary_time,
     int *evaluation_size,
...
/***********************************************************************/
// start recording of the zero signal
/***********************************************************************/
bool check_start_zero_recording
     (int handle,
     int scaleno,
     float recording_time_seconds);

/***********************************************************************/
// abort any previously started recording of the zero signal
/***********************************************************************/
bool check_abort_zero_recording
     (int handle,
     int scaleno);

/***********************************************************************/
// query the progress of any previously started recording of the zero signal
/***********************************************************************/
bool check_query_zero_recording
     (int handle,
     int scaleno,
     float *percentage,
     bool *finished);
...
/***********************************************************************/
// start analyzing the recorded zero signal
/***********************************************************************/
bool check_start_zero_analysis
     (int handle,
     int scaleno,
     int minimum_filter_time,
     int maximum_filter_time,
     float match_percentage,
     bool force_full_search);

/***********************************************************************/
// abort any running zero analysis
/***********************************************************************/
bool check_abort_zero_analysis
     (int handle,
     int scaleno);

/***********************************************************************/
// fetch the progress of an ongoing zero analysis process
/***********************************************************************/
bool check_fetch_zero_analysis_progress
     (int handle,
     int scaleno,
     float *percentage,
     bool *finished);
...
/***********************************************************************/
// fetch the result of a successfully finished zero analysis
/***********************************************************************/
bool check_fetch_zero_analysis_result
     (int handle,
     int scaleno,
     int *filter_time,
     int *zero_settings,
...
#else
  #binary "check_e_driver_V1.5.10.obj"
#endif
