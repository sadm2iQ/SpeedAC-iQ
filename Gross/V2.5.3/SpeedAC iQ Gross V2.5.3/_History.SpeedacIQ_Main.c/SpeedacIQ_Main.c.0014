...
/**************************************************************************** 
  Variables
*****************************************************************************/
#include "SpeedACiQ_Gross_V2.5.1.h"
...
//------------------\\
{
      ...
      decimal   KnockingTime%5.2;
      decimal   EvacuationTime%4.2;
      ...
      decimal   EvacuationDelay%4.2;
      decimal   LanceBlowOffTime%4.2;
      ...
      decimal   BFTime%6.3;
      bool      BulkMode;
      decimal   FeedDelayTime%4.2;
      decimal   PostFillingTime%4.2;
      decimal   BagReleaseDelay%4.2;
      decimal   AspirationTime%4.2;
...
nov double  ProdTotalWeightNOV[2];
nov decimal ProdLastCheckedNOV%7.2[2], ProdLastWeightNOV%7.3[2];
...
decimal ProdLastChecked%7.2[2], ProdLastWeight%7.3[2];
...
decimal      ProdBulkDuration%6.2[2], ProdDribbleDuration%6.2[2],ProdCycleDuration%6.2[2], ProdCycleUnitsPerMin%6.2[2];
float        FastCutoff,SlowCutoff;
bool         TopUp;
...
bool         StartHMI, StopHMI, OffHMI, DischargeHMI, HopperHMI, ResetDataHMI,ResetBatchHMI, ResetLifetimeDataHMI, CheckWeightHMI, AutoTuneHMI, AutoMode; 
...
decimal   NominalWeightMem%6.2,OverfillMem%6.2,ActuatorPosMem%6.2;
int       SlowFilterSizeMem,FastFilterSizeMem,FlashFilterSizeMem,StdFilterSizeMem,StreamRegConfigMem;
float     iQFlSwitchWeightMem,iQFlReduceOutputMem,iQFlControlTimeMem,req_slow_fill_timeMem,iQDeadTimeRead;
...
bool WriteUSBStick = OFF,Synchro_Test,TestFifo;
...
int          MaxNumbOfIO;    
int  TEST_BUTTON[10], TEST_ID[85], TEST_VALUE[85], TEST_CHECK[85], TEST_POINTER[85], TEST_STATE[85], TEST_SELECT[85], TEST_UNITS[85], Test_No, Test_PW;
...
float ActuatorPosition, ActuatorLivePos;

//Temp DORJ2 Test vars
bool Continuous_Output, AutoPage_FS, RT_AutoPage;
bool LowLevel[2], ResetRegistryKey, PrintBypassed;
...
bool IO_Input[IO_ARRAY], IO_Enable[IO_ARRAY], IO_State[IO_ARRAY], IO_Force[IO_ARRAY],IO_TestState[IO_ARRAY], IO_EnableBool[IO_ARRAY], Recipe_Grayout[49], MP_Grayout[37], MC_Grayout[37], 
...
int     SeqMaster, SeqFeeder, SeqFeedSTD, SeqKicker, SeqLockingDevice, SeqLance;
...
enum {Scale1CmpBulk=1,Scale1CmpInterm=2,Scale1CmpDribble=3,Scale1CmpCatchGate=4,Scale1CmpCatchGateInterm=5,
      Scale1CmpAout1=6,Scale1CmpAout2=7}
bool Scale1CmpBulkActive,Scale1CmpIntermActive,Scale1CmpDribbleActive,Scale1CmpCatchGateActive,Scale1CmpCatchGateIntermActive;      
bool IsSettled[2],IsZero[2],FeederBWithAout;
bool SampleBag; //SADM2
...
bool SlowCycleOccured,DischReqInputOn,IsCleanoutDone,CleanoutWeighment;
...
decimal TargetWeight%6.2[2],CutoffCorrOffset%7.3[2];
...
float       CPSmSwitchTimeMem,CPSmSwitchTimePct=0.85;
bool        LearniQSmartParam,LearniQSmartParamMem,LearniQFlashParam,iQFeedDone,LearniQSmartToSave,LearniQFlashToSave;
bool        LearnSmartPlusParam,SetSmartPlusAnalog;
bool        InitiQSmart,InitiQFlash,iQOutputEnabled,InitPLoop,InitCPSmSwitchTime,InitSmartPlus;  
bool        SaveiQFeedParamFlag,SmartPlusSetDribbleCutoff,SmartPlusUseDribbleCutoff=OFF;
int         NbrOf1stLearned;
...
float StartFeedingDelay,Aout2SwitchVal;
      ...
      switch (type)
      {
        ...
        case EVENT_TYPE_BUTTON_PRESS:   
             ...
             { 
                ...
                case NAV_FCT_BUTTON_8 :  // Data table
                            WeightDisplayInfo(No_Forcing);          
                            WeighingDataPopup();       
                            break;  
...
Simul = OFF;
  ...
  if (fifo_used (FIFO_UNITREC_SHARED) + fifo_free (FIFO_UNITREC_SHARED) == 0)
  {
    ...
    fifo_init (FIFO_UNITREC_SHARED, FIFO_UNITREC_SIZE, ON); //ON = persistent
  }
  
  if (fifo_used (FIFO_UNITREC_USB) + fifo_free (FIFO_UNITREC_USB) == 0)
  {
    // fifo has zero size, so it needs to be initialized for the first time
    fifo_init (FIFO_UNITREC_USB, FIFO_UNITREC_SIZE, ON); //ON = persistent
  }
#else
  fifo_init (FIFO_UNITREC_SHARED, FIFO_UNITREC_SIZE, OFF);
  fifo_init (FIFO_UNITREC_USB,    FIFO_UNITREC_SIZE, OFF);
