...
void InitVarMain(void)
{
   ...
   iQFeedSettledCnt=0;
   IO_Change = OFF;
   step_slow_fill_percentage = 0.25;/* 25% */
...
int min;
 ...
 min = IQFEEDSETTLEDNBR;
    ...
    if(WP.FeedAlgoType == FeedGHSSmartPlus)
     {
      min = 1;
      if((CountChar(WP.CPSmSlowAvgPoints,'.') >= min && CountChar(WP.CPSmSlowItpPoints,':') >= min && WP.CPSmSwitchTime!=0)
         && (!WP.TuneiQFeed || (WP.TuneiQFeed && FirstScanCheckLearn)))
       { 
        CPSmSlowAvgPointsMem = WP.CPSmSlowAvgPoints;
        CPSmSlowItpPointsMem = WP.CPSmSlowItpPoints;
        CPSmSwitchTimeMem    = 0; //WP.CPSmSwitchTime;
       }
      else
       {
        learnsmart=ON;
    ...
    switch (ScrnNumbNav)
    {
      case SCREEN_AUTOMATIC: 
              ...
              if ( tmpBTN6 != BTN6 || forced )
              {
                ...
                {
                   case DischOff:   widget_image (NAV_FCT_BUTTON_6, BTN_DISCH_OFF);   break;
                   case DischOn:    widget_image (NAV_FCT_BUTTON_6, BTN_DISCH_ON);    break; 
      ...
      case SCREEN_RECIPES: 
              ...
              if (Scrn_No==SCREEN_IOCFG_OUT1 || Scrn_No==SCREEN_IOCFG_OUT2 || Scrn_No==SCREEN_IOCFG_OUT3 || Scrn_No==SCREEN_IOCFG_OUT4 || Scrn_No==SCREEN_IOTEST_OUT1 || Scrn_No==SCREEN_IOTEST_OUT2 || Scrn_No==SCREEN_IOTEST_OUT3 || Scrn_No==SCREEN_IOTEST_OUT4) 
...
void RefreshGrayout(void)
{
   ...
   Recipe_Grayout[12]= (CurrentLevel < Level_SUP || SeqModeManager != stp_OFF_MODE);
   ...
   Recipe_Grayout[19] = !(WP.FeedAlgoType == FeedStd) && !(WP.FeedAlgoType == FeediQSim);
   Recipe_Grayout[20] = !(MC.FeederType == FeederGHS && (WP.FeedAlgoType == FeedStd || WP.FeedAlgoType == FeediQSim));
   Recipe_Grayout[21] = !(MC.FeederType == FeederGHS && (WP.FeedAlgoType == FeedStd || WP.FeedAlgoType == FeediQSim));
   Recipe_Grayout[22] = (!(IO_Enable[O_CatchGate] || IO_Enable[O_CatchGateInterm]));
   Recipe_Grayout[23] = (!(IO_Enable[O_CatchGate] && MC.FeederType == FeederB && WP.FeedAlgoType == FeedStd));
   Recipe_Grayout[24] = (!(IO_Enable[O_CatchGate] && MC.FeederType == FeederB && WP.FeedAlgoType == FeedStd));
   
   //WP screen #3
   Recipe_Grayout[25] = OFF;
   Recipe_Grayout[26] = OFF;
   Recipe_Grayout[27] = !IO_Enable[O_KnockHammer];
   Recipe_Grayout[28] = !IO_Enable[O_LanceVacuum] || !IO_Enable[O_LowerEvacuationLance] || !IO_Enable[O_RaiseEvacuationLance];
   Recipe_Grayout[29] = !IO_Enable[O_LanceVacuum] || !IO_Enable[O_LowerEvacuationLance] || !IO_Enable[O_RaiseEvacuationLance];
   Recipe_Grayout[30] = !IO_Enable[O_LanceVacuum] || !IO_Enable[O_LowerEvacuationLance] || !IO_Enable[O_RaiseEvacuationLance] || !IO_Enable[O_BlowAir];
   
   Recipe_Grayout[31] = OFF;
   Recipe_Grayout[32] = !IO_Enable[O_ControlledAspiration];
   Recipe_Grayout[33] = (SeqModeManager != stp_OFF_MODE);
   Recipe_Grayout[34]= !(WP.FeedAlgoType == FeediQSmart || WP.FeedAlgoType == FeedGHSSmartPlus);
   Recipe_Grayout[35]= !(WP.FeedAlgoType == FeediQSmart || WP.FeedAlgoType == FeediQFlash || WP.FeedAlgoType == FeedGHSSmartPlus) ;
   Recipe_Grayout[36]= !(WP.FeedAlgoType == FeedStd);
   
   //WP screen #4
   Recipe_Grayout[37] = flexIOoff;
   Recipe_Grayout[38] = flexIOoff;
   Recipe_Grayout[39] = !IO_Enable[O_FlexIO_3];
   Recipe_Grayout[40] = (!IO_Enable[O_KickerActivate]);
   Recipe_Grayout[41] = (!IO_Enable[O_KickerActivate]);
   ...
   MP_Grayout[12]= (!IO_Enable[O_LockingDevice]);
   ...
   MP_Grayout[20]= (!(WP.FeedAlgoType == FeediQSmart) && !(WP.FeedAlgoType == FeedGHSSmartPlus));
   MP_Grayout[21]= (!(WP.FeedAlgoType == FeediQFlash));
   MP_Grayout[22]= (!(WP.FeedAlgoType == FeedStd));
   MP_Grayout[23]= (!(WP.FeedAlgoType == FeediQSmart || WP.FeedAlgoType == FeedGHSSmartPlus));
   ...
   //MC screen #2
   MC_Grayout[13]= (UserSupOrLess || PrintBypassed); /* Production report port */ 
   MC_Grayout[14]= (UserSupOrLess || PrintBypassed); /* Unit record print config */   
   if(WP.FeedAlgoType == FeedStd || WP.FeedAlgoType == FeediQSim) 
   {
      MC_Grayout[15]= 0;
   }
   ...
   MC_Grayout[17]= (UserSupOrLess || PrintBypassed); /* Print port continuous output */ 
   ...
   if(Current_Screen >= SCREEN_RECIPE1 && Current_Screen <= SCREEN_RECIPE4)  
   {
      RefreshWPScreen_No = Current_Screen;
   }
}
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
   ...
   WP.FixedBagWeight      = WPdb.FixedBagWeight;
   WP.MaxBagWeight        = WPdb.MaxBagWeight;
   ...
   WP.KnockingTime       = WPdb.KnockingTime;
   ...
   WP.EvacuationTime      = WPdb.EvacuationTime;
   WP.EvacuationDelay     = WPdb.EvacuationDelay;
   WP.LanceBlowOffTime    = WPdb.LanceBlowOffTime;
   ...
   WP.PostFillingTime       = WPdb.PostFillingTime;
   WP.BagReleaseDelay       = WPdb.BagReleaseDelay;
   WP.AspirationTime        = WPdb.AspirationTime;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
   ...
   WPdb.FixedBagWeight      = WP.FixedBagWeight;
   WPdb.MaxBagWeight        = WP.MaxBagWeight;
   ...
   WPdb.KnockingTime        = WP.KnockingTime;
   ...
   WPdb.EvacuationTime      = WP.EvacuationTime;
   WPdb.EvacuationDelay     = WP.EvacuationDelay;
   WPdb.LanceBlowOffTime    = WP.LanceBlowOffTime;      
   ...
   WPdb.PostFillingTime        = WP.PostFillingTime;
   WPdb.BagReleaseDelay        = WP.BagReleaseDelay;
   WPdb.AspirationTime         = WP.AspirationTime;
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MP.LockingDeviceTime= MPdb.LockingDeviceTime;   
...
/* "Need logic to skip the writing of the following if err = 1 or 2"*/
      ...
      MPdb.LockingDeviceTime= MP.LockingDeviceTime; 
...
void LoadIONames(void)
{
   ...
   IO_Name[3]  = lsub(TX_I_OKTORELEASEBAG);
   ...
   IO_Name[21] = lsub(TX_I_RELEASEACKNOWLEDGE);
   IO_Name[22] = lsub(TX_I_RESETOUTOFTOLALARM);
   IO_Name[23] = lsub(TX_I_LANCEHIGHPOSITION);
   IO_Name[24] = lsub(TX_I_HOLDCYCLE);
   ...
   IO_Name[47] = lsub(TX_O_READYTORELEASEBAG);
   ...
   IO_Name[61] = lsub(TX_O_CLOSINGCNV);
   ...
   IO_Name[71] = lsub(TX_O_BATCHENDED);
   IO_Name[72] = lsub(TX_CLEANOUTCOMPLETED);
   IO_Name[73] = lsub(TX_O_LOWEREVACUATIONLANCE);
   IO_Name[74] = lsub(TX_O_RAISEEVACUATIONLANCE);
   IO_Name[75] = lsub(TX_O_LANCEVACUUM);
   IO_Name[76] = lsub(TX_O_CONTROLLEDASPIRATION);
   IO_Name[77] = lsub(TX_O_BLOWAIR);
   IO_Name[78] = lsub(TX_IO_UNUSED);
   IO_Name[79] = lsub(TX_IO_UNUSED);
   IO_Name[80] = lsub(TX_IO_UNUSED);
   IO_Name[81] = lsub(TX_IO_UNUSED);
   IO_Name[82] = lsub(TX_IO_UNUSED);
   IO_Name[83] = lsub(TX_IO_UNUSED);
   IO_Name[84] = lsub(TX_IO_UNUSED);
}
...
void Init_Fault_Txt (void)
{
   ...
   Fault_Txt[FltG_BagLost] = lsub(TX_FltG_BagLost);
   ...
   Fault_Txt[FltS_EmptyBagTooHeavy] = lsub(TX_FltS_EmptyBagTooHeavy);
  ...
  begindraw();
   ...
   switch(Scrn_No_Temp)
   {
    ...
    case SCREEN_AUTOMATIC:
      // Header
        ...
        // Second Column                               
              ...
              // Third Row Info 
              widget_text       (AUTO_ROW12_ID,   lsub(TX_UNITS_DONE_ID));                                            
              // Fourth Row Info 
              widget_text       (AUTO_ROW13_ID,   lsub(TX_PRESETS_PER_UNIT_ID));
    ...
    case SCREEN_RECIPE1:  
      ...
      //  Header
        ...
        // Second Column                               
              ...
              // Fourth Row Info 
              widget_text       (RECIPE_ROW10_ID, lsub(TX_FIXEDBAGWEIGHT));
              widget_text       (RECIPE_ROW10_SELECT, lsub(TX_OFF), 0);
              widget_text       (RECIPE_ROW10_SELECT, lsub(TX_ON), 1);     
                                                         
              // Fifth Row Info 
              widget_text       (RECIPE_ROW11_ID, lsub(TX_MAXBAGWEIGHT));
              widget_label      (RECIPE_ROW11_INPUT, lsub(TX_MAXBAGWEIGHT));                                             
              // Sixth Row Info 
              widget_text       (RECIPE_ROW12_ID, lsub(TX_WPUNITSELECTION));
              widget_text       (RECIPE_ROW12_SELECT, lsub(TX_WPUNITKG), 0);
              widget_text       (RECIPE_ROW12_SELECT, lsub(TX_WPUNITLB), 1);
     break;
    ...
    case SCREEN_RECIPE2:  
      ...
      //  Header
        ...
        // Second Column                               
              ...
              // First Row Info     
              widget_text       (RECIPE_ROW19_ID,     lsub(TX_WPSLOWCYCLEEND));
              widget_text       (RECIPE_ROW19_SELECT, lsub(TX_WPHOLD_CYCLE), HOLD_CYCLE);
              widget_text       (RECIPE_ROW19_SELECT, lsub(TX_WPEND_CYCLE), END_CYCLE); 
              widget_text       (RECIPE_ROW19_SELECT, lsub(TX_WPDRIBBLEEND), DRIBBLE_CYCLE);       
              // 2nd Row Info      
              widget_text       (RECIPE_ROW20_ID,    lsub(TX_WPVIBRATIONAMPLITUDE));
              widget_label      (RECIPE_ROW20_INPUT, lsub(TX_WPVIBRATIONAMPLITUDE));                                
              // Third Row Info   
              widget_text       (RECIPE_ROW21_ID,    lsub(TX_WPVIBRATIONPERIOD));
              widget_label      (RECIPE_ROW21_INPUT, lsub(TX_WPVIBRATIONPERIOD));                                          
              // Fourth Row Info 
              widget_text       (RECIPE_ROW22_ID,    lsub(TX_WPCATCHGATEDELAY));
              widget_label      (RECIPE_ROW22_INPUT, lsub(TX_WPCATCHGATEDELAY));                                            
              // Fifth Row Info  
              widget_text       (RECIPE_ROW23_ID,    lsub(TX_WPFEEDDELAYTIME));
              widget_label      (RECIPE_ROW23_INPUT, lsub(TX_WPFEEDDELAYTIME)); 
              // Sixth Row Info  
              widget_text       (RECIPE_ROW24_ID,    lsub(TX_WPPOSTFILLINGTIME));
              widget_label      (RECIPE_ROW24_INPUT, lsub(TX_WPPOSTFILLINGTIME));                                      
    ...
    case SCREEN_RECIPE3:  
      ...
      //  Header
        ...
        // First Column                                
              ...
              // Fourth Row Info 
              widget_text       (RECIPE_ROW28_ID,    lsub(TX_WPEVACUATIONDELAY));
              widget_label      (RECIPE_ROW28_INPUT, lsub(TX_WPEVACUATIONDELAY));
              // 5th Row Info 
              widget_text       (RECIPE_ROW29_ID,    lsub(TX_WPEVACUATIONTIME));
              widget_label      (RECIPE_ROW29_INPUT, lsub(TX_WPEVACUATIONTIME));
              // 6th Row Info 
              widget_text       (RECIPE_ROW30_ID,    lsub(TX_WPLANCEBLOWOFFTIME));
              widget_label      (RECIPE_ROW30_INPUT, lsub(TX_WPLANCEBLOWOFFTIME));    
              
        // Second Column                               
                                              
              // First Row Info 
              widget_text       (RECIPE_ROW31_ID,    lsub(TX_WPBAGRELEASEDELAY));
              widget_label      (RECIPE_ROW31_INPUT, lsub(TX_WPBAGRELEASEDELAY));
              // 2nd Row Info
              widget_text       (RECIPE_ROW32_ID,    lsub(TX_WPASPIRATIONTIME));
              widget_label      (RECIPE_ROW32_INPUT, lsub(TX_WPASPIRATIONTIME));
              // 3rd Row Info 
              widget_text       (RECIPE_ROW33_ID, lsub(TX_WPFEEDALGOTYPE));
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOSTD), 0);
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOSMARTIQ), 1);
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOFLASHIQ), 2);
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOSIMIQ), 3);
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOPLOOP), 4);
              widget_text       (RECIPE_ROW33_SELECT, lsub(TX_WPALGOGHSSMARTPLUS), 5);                                              
              // 4rd Row Info 
              widget_text       (RECIPE_ROW34_ID, lsub(TX_SMARTFEEDADJUSMENT));
              widget_text       (RECIPE_ROW34_SELECT, lsub(TX_WPSFF0), 0);
              widget_text       (RECIPE_ROW34_SELECT, lsub(TX_WPSFF1), 1);
              widget_text       (RECIPE_ROW34_SELECT, lsub(TX_WPSFF2), 2);
              widget_text       (RECIPE_ROW34_SELECT, lsub(TX_WPSFF3), 3);
              widget_text       (RECIPE_ROW34_SELECT, lsub(TX_WPSFF4), 4);
              // 5th Row Info 
              widget_text       (RECIPE_ROW35_ID,     lsub(TX_WPTUNEIQFEED));
              widget_text       (RECIPE_ROW35_SELECT, lsub(TX_OFF), 0);
              widget_text       (RECIPE_ROW35_SELECT, lsub(TX_ON), 1);
              // 6th Row Info 
              widget_text       (RECIPE_ROW36_ID,     lsub(TX_WPBULKMODE));
              widget_text       (RECIPE_ROW36_SELECT, lsub(TX_BYWEIGHT), ByWeight);
              widget_text       (RECIPE_ROW36_SELECT, lsub(TX_BYTIME), ByTime);          
     break;      
    ...
    case SCREEN_RECIPE4:  
      ...
      //  Header
        ...
        // First Column                                
              ...
              // 4th Row Info 
              widget_text       (RECIPE_ROW40_ID,    lsub(TX_WPKICKERACTIVATETIME));
              widget_label      (RECIPE_ROW40_INPUT, lsub(TX_WPKICKERACTIVATETIME)); 
              // 5th Row Info 
              widget_text       (RECIPE_ROW41_ID,    lsub(TX_WPKICKERCENTERTIME));
              widget_label      (RECIPE_ROW41_INPUT, lsub(TX_WPKICKERCENTERTIME));
    ...
    case SCREEN_INPUT_PAGE1:
              ...
              widget_text       (IO_ID_ROW3,     lsub(TX_I_OKTORELEASEBAG));
              widget_label      (IO_INPUT_ROW3,  lsub(TX_I_OKTORELEASEBAG));
    ...
    case SCREEN_INPUT_PAGE2:
              ...
              widget_text       (IO_ID_ROW9,     lsub(TX_I_RELEASEACKNOWLEDGE));
              widget_label      (IO_INPUT_ROW9,  lsub(TX_I_RELEASEACKNOWLEDGE));
              widget_text       (IO_ID_ROW10,    lsub(TX_I_RESETOUTOFTOLALARM));
              widget_label      (IO_INPUT_ROW10, lsub(TX_I_RESETOUTOFTOLALARM));
              widget_text       (IO_ID_ROW11,    lsub(TX_I_LANCEHIGHPOSITION));
              widget_label      (IO_INPUT_ROW11, lsub(TX_I_LANCEHIGHPOSITION));
              widget_text       (IO_ID_ROW12,    lsub(TX_I_HOLDCYCLE));
              widget_label      (IO_INPUT_ROW12, lsub(TX_I_HOLDCYCLE));
     break;
    ...
    case SCREEN_OUTPUT_PAGE1:
              ...
              widget_text       (IO_ID_ROW3,     lsub(TX_O_BAGREQUEST));
              widget_label      (IO_INPUT_ROW3,  lsub(TX_O_BAGREQUEST));
              ...
              widget_text       (IO_ID_ROW11,    lsub(TX_O_BAGRELEASE));
              widget_label      (IO_INPUT_ROW11, lsub(TX_O_BAGRELEASE));
    ...
    case SCREEN_OUTPUT_PAGE2:
              ...
              widget_text       (IO_ID_ROW10,    lsub(TX_O_READYTORELEASEBAG));
              widget_label      (IO_INPUT_ROW10, lsub(TX_O_READYTORELEASEBAG));
    ...
    case SCREEN_OUTPUT_PAGE3:
              ...
              // First Row Info 
              widget_text       (IO_ID_ROW1,     lsub(TX_O_CLOSINGCNV));
              widget_label      (IO_INPUT_ROW1,  lsub(TX_O_CLOSINGCNV));
              ...
              widget_text       (IO_ID_ROW11,    lsub(TX_O_BATCHENDED));
              widget_label      (IO_INPUT_ROW11, lsub(TX_O_BATCHENDED));
     ...
     case SCREEN_OUTPUT_PAGE4:
              ...
              // First Row Info 
              widget_text       (IO_ID_ROW1,     lsub(TX_O_LOWEREVACUATIONLANCE));
              widget_label      (IO_INPUT_ROW1,  lsub(TX_O_LOWEREVACUATIONLANCE));
              widget_text       (IO_ID_ROW2,     lsub(TX_O_RAISEEVACUATIONLANCE));
              widget_label      (IO_INPUT_ROW2,  lsub(TX_O_RAISEEVACUATIONLANCE));
              widget_text       (IO_ID_ROW3,     lsub(TX_O_LANCEVACUUM));
              widget_label      (IO_INPUT_ROW3,  lsub(TX_O_LANCEVACUUM));
              widget_text       (IO_ID_ROW4,     lsub(TX_O_CONTROLLEDASPIRATION));
              widget_label      (IO_INPUT_ROW4,  lsub(TX_O_CONTROLLEDASPIRATION));
              widget_text       (IO_ID_ROW5,     lsub(TX_O_BLOWAIR));
              widget_label      (IO_INPUT_ROW5,  lsub(TX_O_BLOWAIR));
              widget_text       (IO_ID_ROW6,     lsub(TX_O_LOCKINGDEVICE));
              widget_label      (IO_INPUT_ROW6,  lsub(TX_O_LOCKINGDEVICE));
    ...
    case SCREEN_MP1:  
      ...
      /* ******************************** */ 
        ...
        // Second Column                               
              ...
              // First Row Info 
              widget_text       (MP_ROW7_ID,    lsub(TX_MPSTABTIMELIMIT));
              widget_label      (MP_ROW7_INPUT, lsub(TX_MPSTABTIMELIMIT));
    ...
    case SCREEN_MC1:  
      ...
      /* ******************************** */ 
        ...
        // Second Column                               
              ...
              // 2nd Row Info 
              // Third Row Info 
              widget_text       (MC_ROW8_ID,     lsub(TX_MCBAGHOLDER));
              widget_text       (MC_ROW8_SELECT, lsub(TX_WPNO), 0);
              widget_text       (MC_ROW8_SELECT, lsub(TX_WPYES), 1);
    ...
    case SCREEN_MC2:  
      ...
      /* ******************************** */ 
        ...
        // Second Column                               
              ...
              // First Row Info 
              widget_text       (MC_ROW19_ID,     lsub(TX_MCDRIBBLEOUTONINBULK));
              widget_text       (MC_ROW19_SELECT, lsub(TX_OFF), 0);
              widget_text       (MC_ROW19_SELECT, lsub(TX_ON), 1);
        
              // 2nd Row Info 
              widget_text       (MC_ROW20_ID,    lsub(TX_MCPLANTLINENAME));
              widget_label      (MC_ROW20_INPUT, lsub(TX_MCPLANTLINENAME));  
                                               
              // Third Row Info 
              widget_text       (MC_ROW21_ID,    lsub(TX_MCSERIALNBR));
              widget_label      (MC_ROW21_INPUT, lsub(TX_MCSERIALNBR));
                                            
              // Fourth Row Info 
              widget_text       (MC_ROW22_ID,    lsub(TX_MCCUSTOMERNAME));
              widget_label      (MC_ROW22_INPUT, lsub(TX_MCCUSTOMERNAME));
                                               
              // Fifth Row Info 
              widget_text       (MC_ROW23_ID,    lsub(TX_MCCUSTOMERLOCATION)); 
              widget_label      (MC_ROW23_INPUT, lsub(TX_MCCUSTOMERLOCATION)); 
    ...
    case SCREEN_REPORT:   
      ...
      // Header
        ...
        // Second Column                               
              ...
              // First Row Info 
              widget_text       (REPORT_ROW8_ID, lsub(TX_ID_TOTAL_UNITS));
        
              // 2nd Row Info 
              widget_text       (REPORT_ROW9_ID, lsub(TX_ID_TOTAL_WEIGHT));
