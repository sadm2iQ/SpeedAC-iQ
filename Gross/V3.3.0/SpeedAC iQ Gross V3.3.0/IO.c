...
void RefreshIOTestLoop (int MaxButtons, int id, int io_num, int state, int pointer, int IOpointer )
{
   ...
   {
      beginrow(); // First row 
         if(!IO_Address[IOpointer+1] && !IO_Enable[IOpointer+1]) 
            widget_text (TEST_VALUE[i], lsub(TX_IO_NULL));
         else 
            widget_text (TEST_VALUE[i], trim(IO_Address[IOpointer+1])); 
         
         if(IO_TestState[IOpointer+1]) 
            widget_text (TEST_STATE[i], lsub(TX_IO_ON));
         else
            widget_text (TEST_STATE[i], lsub(TX_IO_OFF)); 
            
         if(IO_Force[IOpointer+1])
            widget_text (TEST_CHECK[i], ON);
         else
            widget_text (TEST_CHECK[i], OFF); 
 ...
 begindraw(); 
  ...
  switch (Scrn_No)
  {
    ...
    case SCREEN_IOTEST_OUT1 :    
                // First Column  
                 widget_text (IO_SEARCH_ID, concat(space(1), lsub(TX_OUTPUTTEST_PAGE),space(5), "1", space(1), TX_FSLASH, space(1), "4",liveweight));                             
     ...
     case SCREEN_IOTEST_OUT2 :    
                // First Column  
                 widget_text (IO_SEARCH_ID, concat(space(1), lsub(TX_OUTPUTTEST_PAGE),space(5), "2", space(1), TX_FSLASH, space(1), "4",liveweight));                             
     ...
     case SCREEN_IOTEST_OUT3 :    
                // First Column   
                 widget_text (IO_SEARCH_ID, concat(space(1), lsub(TX_OUTPUTTEST_PAGE),space(5), "3", space(1), TX_FSLASH, space(1), "4",liveweight));                           
     ...
     case SCREEN_IOTEST_OUT4 :    
             // First Column   
              widget_text (IO_SEARCH_ID, concat(space(1), lsub(TX_OUTPUTTEST_PAGE),space(5), "4", space(1), TX_FSLASH, space(1), "4",liveweight));                           
       ...
       // Layout of both IO columns
        ...
        // Layout of second column on the right 
          ...
          //  beginrow(); // Fifth row 
            ...
            //  CreateIOField         (IO_ID_ROW11,    CL_PT_BLUE, CL_PT_WHITE, OFF, "Catch gate dribble vibration test", FT_IOPAGES_SIZE, OFF, WD_IO_ANALOG_NAME2, HT_IO_HEADER, No_Margin, widget_locked );
             // CreatePromptField(IO_ID_ROW11,    CL_PT_BLUE, OFF, "Catch Gate Dribble Vibration Test", FT_STANDARD_SIZE, WD_IO_ANALOG_NAME, HT_IO_HEADER, No_Margin, widget_locked);
           //   CreateOnOffBoxField   (IO_INPUT_ROW11, 25, WIDGET_TEXT_ALIGN_CENTER, ON, BTN_CHECKON_SMALL, BTN_CHECKOFF_SMALL);
              
            //  CreateIOField         (IO_ID_ROW11,    CL_PT_BLUE, CL_PT_WHITE, OFF, "Catch gate dribble vibration", FT_IOPAGES_SIZE, OFF, WD_IO_ANALOG_NAME, HT_IO_HEADER, No_Margin, widget_locked );
            //  CreateOutputDataField (IO_INPUT_ROW10, FT_RTC_BOLD, FT_STANDARD_SIZE, WIDGET_TEXT_ALIGN_CENTER, WD_IO_ADDRESS, HT_IO_ADDRESS, widget_locked);
            //  CreateInputIO         (IO_STATE_ROW10, FT_RTC_BOLD, INPUT_FORMAT_DECIMAL, 5, 1, lsub(TX_IO_ENTER_ANALOG), WD_IO_ANALOG_VALUE, HT_IO_ADDRESS, widget_locked);
            //  CreateIOField         (IO_UNIT_ROW10,  CL_PT_BLUE, CL_PT_WHITE, ON, "%", FT_IOPAGES_SIZE, OFF, WD_IO_PERCENT, HT_IO_HEADER, No_Margin, widget_locked );    
        //    endrow();
...
void RefreshAnalogTestScreen(int Scrn_No)
{
   ...
   if(iQFeedAlgorithmName != IQ_FEEDING_ALGORITHM_NONE) SetiQFeedAlgorithm (IQ_FEEDING_ALGORITHM_NONE);
   liveweight = concat(space(5),trim(adj(NetWeight[Scale1],10,dp))," ",WP.Unit);
   result = iQGetCurrentVoltage(SCALE_CHANNEL,SCALE_NUMBER,voltage_mv);
 ...
 begindraw(); 
                ...
                // Analog Input 
                  if(AINActive[AIO1]) AINValue[AIO1] = ActuatorPosition; //ain(AIO1);
                  ...
                  widget_text   (IO_STATE_ROW3, adj(NetWeightHiRes[Scale1],6,dp+1));
                  ...
                  widget_text   (IO_STATE_ROW4, adj(voltage_mv,7,5));
