...
//Refresh Fault Screen 
    ...
    if (DischargeStateFlag != Done && Current_Screen == SCREEN_AUTOMATIC)
    {
      ...
      {                    
        ...
        case DischOff:  widget_image (NAV_FCT_BUTTON_6, BTN_DISCH_OFF);
                           break;
                           
        case DischOn:   widget_image (NAV_FCT_BUTTON_6, BTN_DISCH_ON);
                           break; 
  ...
  //Operation Task Heart Beat monitoring   
   ...
   if((WP.No>0 && WP.No<=MAX_WP)&& WP.NominalWeight==0)
    {
